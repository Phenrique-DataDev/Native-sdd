...
2. **Opcionais do próprio onboarding (nunca pule em silêncio):**
   ...
   - `-WithLocalAi` — MCP de modelo local via Ollama (offline, sem custo de API); exige Ollama + `uv`.
...
## O que NÃO fazer
- Não rode `-ExtraPlugins`/`-WithLocalAi` sem perguntar antes — opt-in é opt-in, não "padrão oculto".
