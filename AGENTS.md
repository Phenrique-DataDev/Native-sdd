...
1. **Base (sempre):** `.\onboarding\install.ps1` (Windows) / `bash ./onboarding/install.sh` (Linux/macOS) —
...
2. **Opcionais do próprio onboarding (nunca pule em silêncio):**
   - `-ExtraPlugins -Themes <tema>` (sem `-Themes` = todos) — repertório de suplementos
     ...
     os temas; não hardcode uma lista de memória, ela já mudou antes. Os **plugins nascem
     desabilitados** — diga isso ao oferecer: depois de instalar, o usuário liga o que for usar pelo
     `/plugin` e configura MCP pelo `/mcp`.
...
## O que NÃO fazer
- Não rode `-ExtraPlugins`/`-WithHerdr` sem perguntar antes — opt-in é opt-in, não "padrão oculto".
