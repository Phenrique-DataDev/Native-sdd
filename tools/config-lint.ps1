...
.DESCRIPTION
    ...
    .claude/sdd/features/DESIGN_CONFIG_LINT.md.
...
# Padrão seguro de regra de permissão: Tool nu (Read) ou Tool(pattern) — ou o wildcard total "*".
$script:RuleFormat = '^[A-Za-z]+(\(.*\))?$'
# Patterns de allow amplos (auto-aprovação): "*", Tool(*), Tool(:*).
$script:BroadAllow = @('^\*$', '^[A-Za-z]+\(\*\)$', '^[A-Za-z]+\(:\*\)$')
    ...
    if ($null -ne $hooks) {
        if (-not (Test-IsJsonObject $hooks)) {
            ...
            foreach ($evt in $hooks.PSObject.Properties.Name) {
                $entries = $hooks.PSObject.Properties[$evt].Value
