...
.DESCRIPTION
    ...
    `kb-lint.ps1`/`agent-lint.ps1`/`config-lint.ps1`) — NÃO reimplementa parsing de frontmatter/JSON:
      ...
      - LINK   : Invoke-LinkLint (broken-link) — link markdown RELATIVO apontando para arquivo que
                 ...
                 ...E A FRASE ACIMA ERA FALSA PARA METADE DOS PROJETOS (auditoria externa, 2026-09-19).
                 A conferência de 2026-08-16 foi feita só no perfil `full`. No `mvp` o mesmo comando
                 devolvia 51 links quebrados e veredito ISSUES num projeto intocado — o perfil
                 recortava `rules/`+`hooks/` e não tocava em quem os CITA, a começar pelo AGENTS.md.
                 Corrigido na origem (`new-project.ps1` reescreve os contratos por perfil), e a
                 conferência deixou de ser manual: `tools/tests/project-check.Tests.ps1` cria projeto
                 nos DOIS perfis e exige `Verdict = ok`. Uma afirmação de doc carimbada com uma data
                 envelhece; a mesma afirmação com teste ao lado reprova quando deixa de valer.
    ...
    try {
        switch ($Kind) {
            ...
            'link' {
                ...
                # `Targets` vai ao painel: `ok (6 arquivos)` num projeto de 82 se denuncia sozinho.
                # Sem ele, o corte do dot-dir no Unix (LINK_LINT_CEGO_A_DOT_DIR_NO_UNIX) saía `ok`.
                return [pscustomobject]@{ Name = 'link'; Status = (Get-CheckVerdict -Findings $f); Findings = $f; Targets = $md.Count }
            }
            'encoding' {
...
function Format-ProjectCheckReport {
    ...
    foreach ($s in @($Report.Sections)) {
        ...
        # Quantos arquivos a seção olhou, quando ela sabe: um `ok` sobre pouco se denuncia sozinho.
        $scope = if ($s.PSObject.Properties['Targets']) { " ($($s.Targets) arquivos)" } else { '' }
        $lines.Add(("  {0} : {1}{2} | {3} error / {4} warn" -f $label, $s.Status, $scope, $errs, $warns))
...
# --- Guard: rodar este arquivo COMO SCRIPT não é sucesso silencioso -------
# A ausência de entry-point é deliberada (nota acima), mas a consequência não era: `pwsh
# tools/project-check.ps1 -ProjectRoot <proj>` imprimia ZERO byte e saía 0 — a forma canônica de
# "passou". Quem lê a lista de ferramentas e tenta executá-la recebia silêncio e ia embora achando
# que o projeto está conforme. A informação de que isto é biblioteca existia, mas nas duas últimas
# linhas do arquivo, depois de 400 de código.
#
# `$MyInvocation.InvocationName -eq '.'` é exatamente o que distingue o dot-source do /check (que
# precisa continuar funcionando) da execução direta. Mesmo predicado de check.ps1/doctor.ps1.
# exit 2 (e não 1): "uso incorreto", não "verificação reprovada" — quem automatizar em cima disso
# não pode confundir os dois.
if ($MyInvocation.InvocationName -ne '.') {
    Write-Host 'project-check.ps1 e uma BIBLIOTECA, nao um script: rodar direto nao verifica nada.'
    Write-Host 'Use:  . ./tools/project-check.ps1 ; Format-ProjectCheckReport (Get-ProjectCheckReport -Root <projeto>)'
    Write-Host 'Ou rode o /check dentro do projeto, que ja faz isso.'
    exit 2
}
