...
function Invoke-AnalyzerCheck {
    ...
    try {
        ...
        $alvos = @('onboarding', 'tools') | ForEach-Object {
            Get-ChildItem -Path (Join-Path $script:RepoRoot $_) -Recurse -File -Include '*.ps1', '*.psm1', '*.psd1' -ErrorAction SilentlyContinue |
...
function Invoke-CollectorCheck {
    ...
    try {
        ...
        $teste = Join-Path $script:RepoRoot 'docs/apps-script/collector.test.js'
...
if ($MyInvocation.InvocationName -ne '.') {
    ...
    if (-not $sum.AllOk) { Write-Host '(advisory — não bloqueia; exit 0)' }
    exit 0
}
