...
.DESCRIPTION
    ...
    Flags:
      ...
      -SkipBudget     pula o painel advisory de contexto always-on (rules-budget, G8)
...
function Invoke-Check {
    ...
    # Bloco ADVISORY (G8): footprint de contexto always-on. Imprime, mas NÃO entra no veredito (AllOk).
    # Usa o parâmetro LOCAL $Quiet (não $script:Quiet) — leitura inequívoca, independe do caminho de invocação.
    if (-not $SkipBudget -and -not $Quiet) {
        try {
            . (Join-Path $PSScriptRoot 'rules-budget.ps1')
            $b = Invoke-RulesBudget -Quiet
            Write-Host ''
            Write-Host ("ⓘ advisory (não afeta o veredito) — " + $b.Summary)
            if ($b.Advisory) { Write-Host $b.Advisory }
        }
        catch {
            Write-Host "ⓘ advisory: rules-budget indisponível ($($_.Exception.Message))"
        }
    }
