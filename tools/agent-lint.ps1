...
# Heading da seção de regras críticas, nas duas grafias aceitas. UNIR aqui é seguro e foi medido:
# `critical rules` tem 0 ocorrência em todo o corpus pt-BR do repositório (83 arquivos).
$script:RxCriticalRules = 'regras\s+cr[íi]ticas|critical\s+rules'

function Get-CriticalRulesVocabulary {
    <#
    .SYNOPSIS  PURA: heading da seção -> par faça/não-faça DAQUELE idioma. $null se não reconhece.
    .DESCRIPTION
        POR QUE SELEÇÃO E NÃO UNIÃO — e é a diferença entre este lint continuar valendo e virar
        enfeite. O par PT é `fa[çc]a|fazer`; o par EN natural é `\bdo\b`. Unir os dois num predicado
        só (`fa[çc]a|fazer|\bdo\b`) faz o lado "faça" casar português em massa: MEDIDO em 2026-08-28
        sobre os 11 agentes base, **31 falsos positivos DENTRO da própria seção** — "do código", "do
        projeto", "do flaky". Todo agente pt-BR passaria a satisfazer `hasDo` sem escrever "faça", e
        o warn `critical-rules-incomplete` viraria inerte para a base inteira.

        Ancorar em `^\s*[-*]\s*\**Do\b` derruba para 0 na seção, e é por isso que NÃO foi a escolha:
        seria 0 por sorte, não por construção. O próprio `git-workflow.md` já escreve
        `- **Do PR ao step:**` em português — 4 seções abaixo desta, e a primeira regra pt-BR que
        começasse com "Do X ao Y" o traria para dentro.

        Escolhendo o vocabulário PELO HEADING, o par EN nunca é aplicado a um documento pt-BR: 0
        falso positivo por construção, e o veredito dos 11 agentes atuais fica IDÊNTICO ao de antes
        desta mudança (é o que `agent-lint-bilingue.Tests.ps1` cobra, agente por agente).
    .OUTPUTS   [pscustomobject] { Lang; Do; Dont } ou $null
    #>
    [CmdletBinding()]
    param([Parameter(Mandatory)][AllowEmptyString()][string]$Heading)

    if ($Heading -imatch 'regras\s+cr[íi]ticas') {
        return [pscustomobject]@{ Lang = 'pt'; Do = 'fa[çc]a|fazer'; Dont = 'n[ãa]o\s+fa' }
    }
    if ($Heading -imatch 'critical\s+rules') {
        # `\bdo\b` só é seguro porque este ramo exige heading EN. Nunca mova para o ramo comum.
        return [pscustomobject]@{ Lang = 'en'; Do = '\bdo\b'; Dont = "\bdon'?t\b|\bdo\s+not\b" }
    }
    return $null
}
...
function Get-AgentBodyFindings {
    <#
    .SYNOPSIS  Verifica a seção de corpo "Regras críticas (faça / não faça)" — pt-BR ou inglês.
    ...
    # `Select-Object -First 1`, não `@(...)[0]`: sob `Set-StrictMode -Version Latest` indexar um
    # array vazio LANÇA (IndexOutOfRangeException) em vez de devolver $null — e o caso vazio é
    # justamente o que a linha seguinte existe para tratar (agente sem a seção).
    $heading = Get-MarkdownHeadings -Text $Text | Where-Object { $_ -imatch $script:RxCriticalRules } | Select-Object -First 1
    if (-not $heading) {
        $findings.Add((New-AgentFinding -Severity error -Rule 'missing-critical-rules' -Path $Source `
                    -Message 'sem seção "Regras críticas (faça / não faça)" nem "Critical rules (do / don''t)"'))
        return $findings.ToArray()
    }

    # O idioma do HEADING escolhe o par. Um documento nunca é avaliado com os dois vocabulários.
    $vocab = Get-CriticalRulesVocabulary -Heading $heading
    $body = Get-MarkdownSectionBody -Text $Text -HeadingPattern $script:RxCriticalRules
    # O lado NEGATIVO é recortado do corpo antes de procurar o positivo. Sem isso "Não faça X"
    # satisfaz os DOIS lados -- `fa[çc]a` casa DENTRO de "não faça" -- e um agente que só escreve
    # o que NÃO fazer passa no par completo. Medido em 2026-08-29: vale para pt (`não faça`,
    # `não fazer`) E para inglês (`do not`, que contém `\bdo\b`); só `don't` escapava, por acaso
    # léxico e não por desenho -- o registro do backlog afirmava o ramo `en` imune, e não era.
    #
    # POR QUE O RECORTE, E NÃO UM LOOKBEHIND POR IDIOMA. `(?<!n[ãa]o\s+)` em pt e `\bdo\b(?!\s+not\b)`
    # em inglês resolveriam os mesmos casos, mas são DOIS predicados escritos à mão, um por ramo:
    # a próxima negação aceita em um lado teria de ser lembrada no outro, e a assimetria volta
    # calada. Usando o PRÓPRIO `$vocab.Dont` como recorte, o lado positivo passa a ser, por
    # construção, "o que sobra depois de tirar tudo o que o lado negativo explica" -- nenhum ramo
    # consegue enxergar uma negação que o outro não enxerga, e ampliar `Dont` corrige os dois.
    $hasDont = $body -imatch $vocab.Dont
    $hasDo = ($body -ireplace $vocab.Dont, '') -imatch $vocab.Do
    if (-not ($hasDo -and $hasDont)) {
        $par = if ($vocab.Lang -eq 'en') { 'do/don''t' } else { 'faça/não-faça' }
        $findings.Add((New-AgentFinding -Severity warn -Rule 'critical-rules-incomplete' -Path $Source `
                    -Message "seção `"Regras críticas`" sem par $par explícito"))
    }
    return $findings.ToArray()
}
...
function Get-AgentRelationalFindings {
    ...
    # Elo agente↔skill em DOIS campos: `skills` é o campo OFICIAL do harness (pré-carrega o conteúdo
    # integral da skill no subagente — determinístico, para skill interna sempre presente);
    # `skills_used` é metadado nosso do grafo, para skill opcional/de terceiro (listá-la em `skills`
    # geraria warning de skill ausente em projeto que não a instalou). Validação ADVISORY (warn) —
    # NUNCA barra o CI (skills são runtime/curadas, podem ser global/ausentes). $null => silêncio.
    if ($null -ne $KnownSkills) {
        foreach ($key in @('skills', 'skills_used')) {
            if (-not $Frontmatter.Contains($key) -or [string]::IsNullOrWhiteSpace($Frontmatter[$key])) { continue }
            foreach ($s in (ConvertFrom-InlineList -Value $Frontmatter[$key])) {
                if ($s -notin $KnownSkills) {
                    $findings.Add((New-AgentFinding -Severity warn -Rule 'unknown-skill-used' -Path $Source `
                                -Message "$key aponta a skill fora do inventário: '$s'"))
                }
            }
        }
    }
...
function Invoke-AgentLint {
    ...
    # H9: nomes de skills do projeto (pasta irmã .claude/skills) + repertório de suplementos
    # (tools/supplements.psd1, sibling deste script — dot-source direto via $PSScriptRoot, sem
    # cascata: já estamos dentro de tools/) p/ validar skills_used. NENHUMA das duas fontes
    # resolve => $null => agent-lint NÃO valida skills_used (degrada/silêncio, D-006). Identidade:
    # nome da pasta (skill do projeto) OU 'Name' do catálogo (plugin/skill de suplemento).
    $skillsDir = Join-Path (Split-Path -Parent $Dir) 'skills'
    $hasSkillsDir = Test-Path -LiteralPath $skillsDir -PathType Container
    $localSkills = @()
    if ($hasSkillsDir) {
        $localSkills = @(Get-ChildItem -LiteralPath $skillsDir -Directory -ErrorAction SilentlyContinue | ForEach-Object { $_.Name })
    }
    $supplementsScript = Join-Path $PSScriptRoot 'supplements.ps1'
    $hasSupplements = Test-Path -LiteralPath $supplementsScript -PathType Leaf
    $catalogSkills = @()
    if ($hasSupplements) {
        . $supplementsScript
        $catalogSkills = @(Get-SupplementCatalog -WarningAction SilentlyContinue | ForEach-Object { $_.Name })
    }
    $knownSkills = $null
    if ($hasSkillsDir -or $hasSupplements) {
        $knownSkills = @(@($localSkills) + @($catalogSkills) | Select-Object -Unique)
    }
