...
# simulador de stack (ex.: dbt-simulator) que cumpre o contrato do /simulate. Os 8 demais são
# papéis universais do base. Aditivo/retrocompat (ver DESIGN_SIMULATION.md, D-002).
$script:AgentRoles = @('search', 'review', 'testing', 'vcs', 'security', 'debug', 'validation', 'documentation', 'simulation')
...
function Invoke-AgentLint {
    ...
    foreach ($f in $files) {
        ...
        foreach ($x in (Get-AgentRelationalFindings -Frontmatter $fmMap[$src] -KnownNames $known -Source $src)) {
            $findings.Add($x)
        }
    }
    return $findings.ToArray()
