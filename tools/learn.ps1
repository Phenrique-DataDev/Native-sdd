...
.DESCRIPTION
    ...
    promoção) é runtime do LLM; aqui mora só o que é objetivo:
      ...
      - PÓS-CONDIÇÃO: Test-LessonEnforcement — toda entrada RECÉM-GRAVADA declara `enforced_by`:
                      o mecanismo que reprova sem a prática, ou `none` DECLARADO. As duas perguntas
                      são diferentes — "de onde veio" (origem) e "o que a cobra" (mecanismo) — e por
                      isso são funções separadas, com escopos diferentes: a origem vale para a KB
                      inteira; o mecanismo, só para o que o passo 5 acabou de escrever (retrocompat
                      por construção — entrada legada não vira finding).
...
# --- PURA: lê id + UMA chave de lista do frontmatter (inline OU multilinha) --------------------
function Read-KbListField {
    <#
    .SYNOPSIS
        Parser único das chaves de lista do frontmatter da KB (promoted_from, enforced_by, …).
        Duas cópias da mesma verificação é como o `orphan-posture` nasceu duplicado — aqui a
        forma de ler o frontmatter mora em UM lugar e os leitores públicos delegam.
    .OUTPUTS [pscustomobject] @{ Id; Values=[string[]]; HasKey=[bool] }
        HasKey distingue "chave ausente" (entrada comum) de "chave presente porém vazia"
        (declaração pela metade → finding). Os dois casos reprovam, mas por razões diferentes.
    #>
    [CmdletBinding()]
    param(
    ...
    $empty = [pscustomobject]@{ Id = $null; Values = @(); HasKey = $false }
    ...
    for ($i = 1; $i -lt $lines.Count; $i++) {
        ...
        if ($line -match '^\s*-\s+(.+)$') {                    # item de array multilinha
            ...
            if ($val -and $currentKey -eq $Key) { $values += $val }
            continue
        }
        $idx = $line.IndexOf(':')
        ...
        $currentKey = $k
        if ($k -eq 'id')  { $id = $val.Trim('"', "'") }
        if ($k -eq $Key)  { $hasKey = $true; $values += (ConvertFrom-LearnInlineList $val) }
    }
...
function Read-KbPromotedFrom {
    ...
    $f = Read-KbListField -Path $Path -Key 'promoted_from'
    return [pscustomobject]@{
        Id              = $f.Id
        PromotedFrom    = @($f.Values)
        HasPromotedFrom = $f.HasKey
    }
}

# --- PURA: lê id + enforced_by do frontmatter -------------------------------------------------
function Read-KbEnforcedBy {
    <#
    .SYNOPSIS
        `enforced_by` = o que REPROVA sem a prática promovida. Lista de mecanismos (teste, lint,
        hook, gate) ou o escalar `none` DECLARADO — "nada cobra isto, e nós sabemos".
    .OUTPUTS [pscustomobject] @{ Id; EnforcedBy=[string[]]; HasEnforcedBy=[bool] }
    #>
    [CmdletBinding()]
    param([Parameter(Mandatory)][string]$Path)

    $f = Read-KbListField -Path $Path -Key 'enforced_by'
    return [pscustomobject]@{
        Id            = $f.Id
        EnforcedBy    = @($f.Values)
        HasEnforcedBy = $f.HasKey
    }
}
...
# --- PURA: a prática promovida declara o que a cobra? -----------------------------------------
function Test-LessonEnforcement {
    <#
    .SYNOPSIS
        Pós-condição do passo 6 do /learn: cada entrada RECÉM-GRAVADA declara `enforced_by` —
        o mecanismo que reprova sem a prática, ou o escalar `none` DECLARADO.

        O /learn promove lição a entrada de KB `operations`: PROSA. Sem este campo, uma prática
        coberta por teste e uma prática que ninguém cobra ficam indistinguíveis para quem lê a KB
        seis meses depois — que é a falha desta metodologia batiza como "regra sem mecanismo",
        aplicada justamente ao command cujo propósito é converter lição em prática.

        `none` é resposta LEGÍTIMA e não é derrota: recusar promover sem mecanismo transformaria um
        command nunca-destrutivo em gate, e faria o curador INVENTAR um mecanismo para passar. Um
        `none` honesto vale mais que um teste fabricado (mesma lógica do "N/A declarado" da seção
        `Verificação por mutação` do BUILD_REPORT).

    .PARAMETER Ids
        Os `id` das entradas que o passo 5 ACABOU de gravar. O escopo é ESTE conjunto, nunca a KB
        inteira: entrada legada (promovida antes deste contrato existir) não vira finding — a
        retrocompatibilidade é por CONSTRUÇÃO, não por severidade abaixada.

    .OUTPUTS [pscustomobject[]] findings (vazio = toda entrada nova declarou)
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)][string]$KbDir,
    ...
    $findings = @()
    if (-not (Test-Path -LiteralPath $KbDir -PathType Container)) { return $findings }
    if (@($Ids).Count -eq 0) { return $findings }

    foreach ($e in @(Get-KbInventory -Dir $KbDir)) {
        $f = Read-KbEnforcedBy -Path $e.Path
        if (-not $f.Id -or ($Ids -notcontains $f.Id)) { continue }   # fora do escopo verificado

        if (-not $f.HasEnforcedBy) {
            $findings += [pscustomobject]@{
                Severity = 'error'
                Rule     = 'lesson-without-enforcement'
                Id       = $f.Id
                Message  = "entrada '$($f.Id)' nao declara enforced_by (o que reprova sem a pratica). Nomeie o mecanismo ou declare 'none'."
            }
        }
        elseif (@($f.EnforcedBy).Count -eq 0) {
            $findings += [pscustomobject]@{
                Severity = 'error'
                Rule     = 'lesson-without-enforcement'
                Id       = $f.Id
                Message  = "entrada '$($f.Id)' tem enforced_by vazio — chave presente sem declaracao e pior que ausente (parece preenchida)."
            }
        }
    }
    return @($findings)
...
function Format-LearnReport {
    ...
    if ($Findings -and $Findings.Count -gt 0) {
        ...
        $lines += 'promoção: OK (origem rastreável e mecanismo declarado em toda entrada verificada).'
    }
    return ($lines -join "`n")
}
