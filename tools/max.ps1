...
.DESCRIPTION
    ...
    Estado é FAIL-CLOSED + TTL, e session-bound **quando há session id**: qualquer erro/corrupção/
    staleness ⇒ desligado. O vínculo de sessão é condicional a ambos os lados terem id — sem ele o
    TTL é a única trava (ver a nota em Get-MaxState e o teste MX-15).
...
function Format-MaxNotice {
    ...
    # Validade do flag (do State). O vínculo de sessão SÓ existe quando o harness expôs um id
    # (o command passa `-SessionId <id, se disponível>`); sem ele, `Get-MaxState` não tem com o quê
    # comparar e o TTL vira a única trava. A linha abaixo diz isso em vez de prometer
    # "não persiste entre sessões" — promessa que seria falsa justamente no harness sem id.
    $exp = ConvertTo-LongOrZero (Get-PropOrNull $State 'ExpiresAt')
    ...
    $lines.Add("  Validade      : flag expira ~$validade (TTL); so nao passa a outra sessao se o harness expuser session id")
