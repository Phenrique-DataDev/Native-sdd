...
function Get-OrchestrationStatus {
    ...
    # Ledger das chamadas do predicado (ver Add-OrchestrationModeCall): mora ao lado do STATE, e é
    # lido aqui para o painel poder acusar re-invocação com entradas divergentes.
    $modeCalls = @()
    if (-not [string]::IsNullOrWhiteSpace($StatePath)) {
        $dir = Split-Path -Parent $StatePath
        if ($dir -and (Test-Path -LiteralPath $dir -PathType Container)) {
            $modeCalls = @(Get-OrchestrationModeCall -StateDir $dir)
        }
    }
    ...
    return [pscustomobject]@{
        ...
        # Ledger das chamadas do predicado, lido do mesmo diretório do STATE. Fica no $Status (e não
        # dentro do Format-*) para o painel seguir sendo pura formatação do que recebe.
        ModeCalls        = @($modeCalls)
...
function Get-OrchestrationStateDir {
    <#
    .SYNOPSIS
        PURA (só Test-Path). Onde o STATE desta orquestração deve morar: o diretório canônico em
        `.claude/sdd/orchestration/`, ou o fallback `.sdd-orchestration/` na raiz do projeto.

    .DESCRIPTION
        POR QUE EXISTE (auditoria externa da v0.10.61, Anexo E.3). Medido: o líder leu as duas
        posturas, rodou o predicado, recebeu `completo`, montou o STATE e tentou gravá-lo —
        `Write` negado, porque `.claude/` é caminho protegido do harness. Ele respeitou a negação
        (correto), não contornou por Bash (correto) e parou **antes do primeiro `Agent`**. Saldo:
        US$ 0,4170 e ZERO arquivo, contra US$ 0,5589 e quatro arquivos do mesmo objetivo sem
        comando nenhum. Todo o protocolo — STATE no Passo 2, saída bruta no Passo 3.3 — atravessa
        `.claude/`.

        A CORREÇÃO QUE O RELATÓRIO PROPUNHA NÃO FUNCIONA, e isso foi MEDIDO em 2026-09-19, em
        quatro sessões `claude -p`:
          A) scaffold como entregue                        -> negado
          B) + `permissions.allow` no settings.json        -> a allowlist é IGNORADA:
             "Ignoring N permissions.allow entries: this workspace has not been trusted" — e um
             projeto recém-criado NUNCA passou pelo trust dialog, que é justamente o caso de uso
          C) + trust aceito                                -> `Write(...)` nem é regra válida para
             arquivo ("only Edit(path) rules are"), e com `Edit(.claude/**)` válido a escrita
             CONTINUA negada: a proteção de arquivo sensível vence a allowlist
          D) `--dangerously-skip-permissions`              -> escreveu
        Ou seja: entregar a allowlist no scaffold seria inerte em dois níveis. O caminho
        não-interativo documentado é a flag — e é do usuário decidir usá-la, não nossa.

        O que ESTE fallback compra: a cadeia deixa de morrer no Passo 2. `.sdd-orchestration/` é
        área de ARTEFATO DE TRABALHO, não de configuração, e fica fora de `.claude/` de propósito.
        Custa espalhar estado do SDD para fora da pasta que o organiza — por isso o preferido
        continua sendo o canônico, e o fallback é anunciado, nunca silencioso.
    .OUTPUTS
        [pscustomobject] { Path; IsFallback; Canonical }
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)][string]$ProjectRoot,
        # $true quando o chamador JÁ tentou gravar no canônico e foi negado. Sem isto o fallback
        # viraria o default silencioso — e aí o estado do SDD sairia de `.claude/` sem ninguém pedir.
        [switch]$CanonicalDenied
    )
    $canonical = Join-Path (Join-Path $ProjectRoot '.claude') 'sdd/orchestration'
    if (-not $CanonicalDenied) {
        return [pscustomobject]@{ Path = $canonical; IsFallback = $false; Canonical = $canonical }
    }
    return [pscustomobject]@{
        Path       = (Join-Path $ProjectRoot '.sdd-orchestration')
        IsFallback = $true
        Canonical  = $canonical
    }
}

function Get-OrchestrationLedgerPath {
    # PURA. O ledger fica ao lado do STATE, no diretório da orquestração.
    param([Parameter(Mandatory)][string]$StateDir)
    return (Join-Path $StateDir 'mode-ledger.jsonl')
}

function Add-OrchestrationModeCall {
    <#
    .SYNOPSIS
        Registra UMA chamada de Get-OrchestrationMode (entrada + Mode devolvido) no ledger.

    .DESCRIPTION
        POR QUE ISTO EXISTE, E POR QUE MORA AQUI E NÃO DENTRO DO PREDICADO (auditoria externa da
        v0.10.61, 2026-09-19). Medido em sessão real: o líder rodou
        `Get-OrchestrationMode -Disciplines @('doc') -FitsSingleContext $false -ReductionAttempted $true`
        e recebeu `completo`; rodou A MESMA função de novo com `-FitsSingleContext $true`, recebeu
        `solo`, e foi o `solo` que chegou ao usuário — com a justificativa "o completo também exige
        `-FitsSingleContext $false`, e esse eu não consigo afirmar". A justificativa é razoável; o
        que não existia era mecanismo. Nada no caminho registrava que houve duas chamadas com
        respostas opostas, e quem lê a `Reason` acredita estar vendo um veredito de máquina.

        O `.SYNOPSIS` de `Get-OrchestrationMode` declara a função PURA e sem I/O, e essa propriedade
        é o que a torna testável e previsível — gravar lá dentro a quebraria. Por isso o ledger é do
        CHAMADOR: o command registra, o painel acusa.

        E ele REGISTRA, não trava. Travar a segunda chamada impediria o caso legítimo (o líder
        descobre um fato novo e corrige um sinal) para pegar o caso ruim. O que faltava não era
        impedir o re-roll — era que ele deixasse rastro.
    .OUTPUTS
        [string] caminho do ledger.
    #>
    [CmdletBinding()]
    param(
    ...
    $path = Get-OrchestrationLedgerPath -StateDir $StateDir
    $registro = [ordered]@{
    ...
    # JSONL: uma linha por chamada, append puro. Sem timestamp — o painel é determinístico, e a
    # ORDEM já é a informação (a última chamada é a que o líder reportou).
    Add-Content -LiteralPath $path -Value ($registro | ConvertTo-Json -Compress -Depth 5) -Encoding utf8
    return $path
}

function Get-OrchestrationModeCall {
    # Lê o ledger. Ausente/ilegível -> @() (não sei ≠ não houve, mas aqui o fail-safe é o silêncio:
    # acusar divergência sem dado seria pior que não acusar).
    [CmdletBinding()]
    param([Parameter(Mandatory)][string]$StateDir)
    ...
    foreach ($line in (Get-Content -LiteralPath $path -ErrorAction SilentlyContinue)) {
        ...
        try { $out.Add(($line | ConvertFrom-Json)) } catch { continue }
    }
    return @($out)
}

function Test-OrchestrationModeDivergence {
    <#
    .SYNOPSIS
        PURA. Dadas as chamadas do ledger, houve re-invocação com entradas divergentes?
    .OUTPUTS
        [pscustomobject] { Diverged; Count; Modes }
        Diverged = $true quando há >1 chamada E os sinais informados mudaram entre elas.
    #>
    [CmdletBinding()]
    param([Parameter(Mandatory)][AllowEmptyCollection()][psobject[]]$Calls)

    $c = @($Calls)
    # Duas chamadas IDÊNTICAS não são divergência: repetir o mesmo predicado é ruído, não re-roll.
    $assinaturas = @($c | ForEach-Object {
            '{0}|{1}|{2}|{3}|{4}' -f (@($_.disciplines) -join ','), $_.fitsSingleContext,
            $_.reductionAttempted, $_.isolationRequired, $_.outwardAction
        } | Sort-Object -Unique)
    return [pscustomobject]@{
        Diverged = ($c.Count -gt 1 -and $assinaturas.Count -gt 1)
        Count    = $c.Count
        Modes    = @($c | ForEach-Object { [string]$_.mode })
    }
}
...
function Format-OrchestrationReport {
    ...
    # Retrocompat, igual aos campos acima: $Status montado à mão não tem ModeCalls.
    $modeCalls = @(if ($Status.PSObject.Properties['ModeCalls']) { $Status.ModeCalls } else { @() })
    $div = Test-OrchestrationModeDivergence -Calls $modeCalls
    if ($div.Diverged) {
        [void]$sb.Append("ATENÇÃO: o predicado de regime foi chamado $($div.Count) vezes com entradas DIVERGENTES ($($div.Modes -join ' -> ')) — a função é determinística, a entrada dela não é. Confira se o sinal que mudou é fato novo ou conveniência.`n")
    }
