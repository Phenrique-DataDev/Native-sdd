...
.DESCRIPTION
    ...
    Test-CurationReadiness / Format-CurationReport) — molde, não dot-source.
      ...
      Get-OrchestrationStatus       -> [pscustomobject]     { Objective; Total; Passed; Failed; Pending; Awaiting; Tasks[]; ReadyTasks[]; AwaitingApproval[]; NextTask }
...
function ConvertFrom-OrchestrationState {
    ...
    foreach ($line in $lines) {
        ...
        if ($line -match '^\s*-\s*id\s*:\s*(.+?)\s*$') {
            ...
            $cur = @{ Id = $Matches[1].Trim().Trim('"').Trim("'"); Title = ''; Dod = ''; Model = ''; Type = ''; Deps = @(); Status = 'pending' }
        }
        elseif ($cur) {
...
function Get-OrchestrationStatus {
    ...
    .SYNOPSIS
        ...
        Idempotente (não rebaixa 'passed'). STATE ausente -> estado vazio (NextTask='done').
    .OUTPUTS
        [pscustomobject] { Objective; Total; Passed; Failed; Pending; Awaiting; Tasks[]; ReadyTasks[]; AwaitingApproval[]; NextTask }
        NextTask: <id despachável> | 'awaiting-approval' | 'blocked' | 'done'.
    #>
    [CmdletBinding()]
    param([AllowNull()][string]$StatePath)
