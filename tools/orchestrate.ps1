...
function Get-OrchestrationMode {
    ...
    .PARAMETER FitsSingleContext
        ...
        ANTES de informar $false, faça a pergunta que a medição impõe: UM SCRIPT RESOLVERIA? Material
        com estrutura (mesmo formato, vocabulário fechado, campo parseável) é REDUTÍVEL — e redutível
        cabe numa FERRAMENTA, não num contexto. Rodada 3 do ORCHESTRATE_BASELINE (2026-08-11): três
        ...
        Volume não é a pergunta; irredutibilidade é. Informar $false por TAMANHO cobra o ciclo
        completo por uma fronteira que não existe.
    ...
    if (-not $FitsSingleContext) {
        $motivos = @('o material não cabe no contexto de um agente')
    ...
    if ($sufixos.Count -gt 0) {
        return [pscustomobject]@{
            ...
            Reason  = 'executor + 1 revisor fresh-context · ' + ($porques -join ' · ')
            Signals = $signals
        }
    }
