...
function ConvertFrom-OrchestrationState {
    ...
    .OUTPUTS
        [pscustomobject[]] { Id; Title; Dod; Model; Type; Deps[]; Status } — ordenado por Id.
    #>
    [CmdletBinding()]
    param(
    ...
    foreach ($line in $lines) {
        ...
        if ($line -match '^\s*-\s*id\s*:\s*(.+?)\s*$') {
            ...
            $cur = @{ Id = $Matches[1].Trim().Trim('"').Trim("'"); Title = ''; Dod = ''; Model = ''; Type = ''; Deps = @(); Status = 'pending'; Seen = @{} }
        }
        elseif ($cur) {
            ...
            if ($line -match '^\s*(title|dod|model|type|status|deps)\s*:') {
                $key = $Matches[1]
...
function Get-OrchestrationStatus {
    ...
    .OUTPUTS
        [pscustomobject] { Objective; Total; Passed; Failed; Pending; Awaiting; IgnoredLines; Tasks[]; ReadyTasks[]; AwaitingApproval[]; NextTask }
...
function Get-OrchestrationMode {
    <#
    .SYNOPSIS
        Decide o REGIME antes de decompor: 'solo' | 'minimo' | 'completo'. Puro, sem I/O.
    ...
    .PARAMETER FitsSingleContext
        ...
        $false é a fronteira mais forte que existe: é a hipótese que a orquestração de fato afirma.
    .PARAMETER IsolationRequired
        $true se tasks escrevem nos MESMOS arquivos em paralelo (exige worktree/isolamento).
    ...
    # Fronteiras ESTRUTURAIS -> ciclo completo. Cada uma é verificável antes de decompor, e cada uma
    # nomeia algo que o modo mínimo NÃO consegue fazer: caber, não conflitar, parar para o humano.
    #
    ...
    $fronteiras = @()
    if (-not $FitsSingleContext) { $fronteiras += 'o material não cabe no contexto de um agente' }
    if ($IsolationRequired)      { $fronteiras += 'tasks concorrentes exigem isolamento (worktree)' }

    if ($fronteiras.Count -gt 0) {
        return [pscustomobject]@{
            Mode    = 'completo'
            Reason  = 'fronteira de máquina de estado: ' + ($fronteiras -join ' · ')
            Signals = $signals
        }
    }

    # Ação outward NÃO escala para 'completo'. Ela exige UMA coisa — a parada para o humano — e
    # parada é barata; STATE de N tasks é caro. Acoplar os dois é a mesma forma da fronteira de
    # disciplina que a rodada 2 derrubou: cara amarrada ao barato.
    #
    ...
    if ($OutwardAction) {
        return [pscustomobject]@{
            Mode    = 'minimo+checkpoint'
            Reason  = 'ação outward: executor + 1 revisor fresh-context, e PARADA obrigatória para o humano antes de cada passo que sai da máquina'
            Signals = $signals
        }
    }
    ...
    if ($distinct.Count -ge 2) {
        return [pscustomobject]@{
            Mode    = 'minimo'
            Reason  = "sem fronteira estrutural, mas $($distinct.Count) disciplinas: executor + 1 revisor fresh-context com checklist"
            Signals = $signals
        }
    }
    ...
    return [pscustomobject]@{
        Mode    = 'solo'
        Reason  = 'nenhuma fronteira observável — orquestrar aqui custa sem comprar (ver ORCHESTRATE_BASELINE)'
        Signals = $signals
    }
}
