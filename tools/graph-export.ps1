...
function ConvertTo-GraphCypher {
    ...
    foreach ($n in @($Graph.Nodes)) {
        # Label = Type do nó (Agent | Hub | KbEntry | Skill | Domain | Feature). name/role/description
        # SEMPRE emitidos (mantém o dump dos :Agent/:Hub byte-idêntico ao H4/H7); props extras
        # (Layer/Domain/ContentType/Status/Scope…) emitidas em ordem de definição, só quando não-vazias.
        $props = [System.Collections.Generic.List[string]]::new()
        $props.Add("name: '$(& $esc $n.Id)'")
        $props.Add("role: '$(& $esc $n.Role)'")
        $props.Add("description: '$(& $esc $n.Description)'")
        foreach ($p in $n.PSObject.Properties) {
            ...
            $props.Add("$($p.Name.ToLowerInvariant()): '$(& $esc $p.Value)'")
        }
        $out.Add("CREATE (:$($n.Type) {$($props -join ', ')});")
    }
    foreach ($e in @($Graph.Edges)) {
        $from = & $esc $e.From; $to = & $esc $e.To
        # MATCH agnóstico de label (por name) p/ casar tanto :Agent quanto :Hub (aresta :ORCHESTRATES).
        $out.Add("MATCH (a {name: '$from'}), (b {name: '$to'}) CREATE (a)-[:$($e.Type)]->(b);")
    }
    return ($out -join "`n")
}
