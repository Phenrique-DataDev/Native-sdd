...
.DESCRIPTION
    ...
    ESCOPO (Get-LinkLintTarget): todo `.md` do repositório, MENOS o que o git ignora (reusa
...
function Get-LinkLintTarget {
    ...
    $all = @(Get-ChildItem -LiteralPath $Root -Recurse -File -Filter '*.md' -ErrorAction SilentlyContinue |
        Where-Object { $p = $_.FullName; -not (@($ExcludeFragment) | Where-Object { $p -like "*$_*" }) } |
