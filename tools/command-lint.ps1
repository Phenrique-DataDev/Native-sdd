...
# Tools que ESCREVEM. Fonte unica da regra `readonly-sem-contrato`; qualquer uma delas fora do
# `disallowed-tools` de um command que se declara read-only deixa o contrato pela metade.
$script:WriteTools = @('Edit', 'Write', 'NotebookEdit')

function Get-FrontmatterBlock {
    <# .SYNOPSIS  O TEXTO do frontmatter YAML (sem os '---'), ou '' se nao houver. PURA. #>
    [CmdletBinding()]
    param([Parameter(Mandatory)][AllowEmptyString()][string]$Text)
...
function Get-FrontmatterToolList {
    <#
    .SYNOPSIS
        Os nomes de tool de uma chave do frontmatter (`allowed-tools`/`disallowed-tools`). PURA.
    .DESCRIPTION
        A plataforma aceita tres formas para essas chaves (doc oficial, `skills#frontmatter-reference`):
        string separada por espaco, string separada por virgula, ou lista YAML. Aceitamos as tres --
        um lint que so entendesse uma delas reprovaria frontmatter valido, que e pior que nao checar.

        So o NOME da tool importa aqui: `Bash(git status:*)` vira `Bash`. O escopo do Bash nao muda
        se a tool escreve -- quem responde por isso e o `permissions` do settings, nao este lint.
    .OUTPUTS  [string[]] (vazio = chave ausente ou sem valor)
    #>
    [CmdletBinding()]
    param(
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        $m = [regex]::Match($lines[$i], "^\s*$([regex]::Escape($Key)):\s*(.*)$")
        if (-not $m.Success) { continue }

        $inline = $m.Groups[1].Value.Trim().Trim('"').Trim("'").Trim()
        if ($inline) {
            # forma inline: `Read Grep Glob`, `Read, Grep`, ou `[Read, Grep]`.
            # NAO usar split por espaco/virgula: o escopo do Bash CONTEM os dois
            # (`Bash(git status:*), Read` viraria `Bash(git` + `status:*)` + `Read`).
            # Casa o token inteiro: nome + parenteses opcional que engole o que houver dentro.
            foreach ($t in [regex]::Matches($inline.Trim('[', ']'), '[\w.-]+(?:\([^)]*\))?')) {
                if ($t.Value) { $vals.Add($t.Value) }
            }
        }
        else {
            # forma lista YAML: itens `- Read` nas linhas seguintes, ate a proxima chave
            for ($j = $i + 1; $j -lt $lines.Count; $j++) {
                $item = [regex]::Match($lines[$j], '^\s*-\s*(.+?)\s*$')
                if (-not $item.Success) { break }
                $vals.Add($item.Groups[1].Value.Trim().Trim('"').Trim("'"))
            }
        }
        break
    }

    # `Bash(git status:*)` -> `Bash`; descarta o que sobrar vazio.
    return @($vals | ForEach-Object { ($_ -split '\(')[0].Trim() } | Where-Object { $_ })
}

function Get-ReadOnlyContractFindings {
    <#
    .SYNOPSIS
        Command que se declara read-only (`metadata.contract: read-only`) precisa do contrato
        EXECUTAVEL, nao so da prosa: `disallowed-tools` cobrindo Edit/Write/NotebookEdit.
    .DESCRIPTION
        POR QUE `metadata.contract` E NAO A PROSA (medido nos 30 commands, 2026-08-16): o predicado
        "a description diz read-only" erra nos dois sentidos. `/peers` diz "read-only por padrao" e
        ESCREVE (poda stale, marca recado como lido, envia mensagem) -- seria falso-positivo;
        `/doctor` e `/telemetry` sao read-only integrais e NAO tem a palavra na description -- seriam
        falso-negativos. Declaracao explicita nao tem essa ambiguidade, e `metadata` e o campo que a
        plataforma reserva para "your own tooling" (o harness nao age sobre o conteudo dele).

        POR QUE `disallowed-tools` E NAO `allowed-tools`: a doc e explicita -- `allowed-tools` "does
        not restrict which tools are available: every tool remains callable". Ele CONCEDE (roda sem
        prompt de permissao no turno). O campo que remove a tool do pool e `disallowed-tools`. Um
        contrato read-only escrito com `allowed-tools` nao impede escrita nenhuma -- parece protecao
        e nao e, que e o pior modo de falha desta familia.

        A segunda regra (`allowed-tools` nao pode conceder o que `disallowed-tools` remove) cobre o
        contrato auto-contraditorio: conceder Write e remove-lo no mesmo arquivo e sinal de edicao
        pela metade, e o leitor nao tem como saber qual das duas linhas vale.
    .OUTPUTS  [pscustomobject[]] (vazio = conforme, ou command que nao se declara read-only)
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)][AllowEmptyString()][string]$Text,
    ...
    $findings = [System.Collections.Generic.List[object]]::new()
    $fm = Get-FrontmatterBlock -Text $Text
    if (-not $fm) { return $findings.ToArray() }

    # So cobra de quem se declara. Nao declarar nao e violacao: a maioria dos commands escreve.
    if ($fm -notmatch '(?m)^\s+contract:\s*["'']?read-only["'']?\s*$') { return $findings.ToArray() }

    $disallowed = Get-FrontmatterToolList -Frontmatter $fm -Key 'disallowed-tools'
    $allowed = Get-FrontmatterToolList -Frontmatter $fm -Key 'allowed-tools'

    $missing = @($script:WriteTools | Where-Object { $_ -notin $disallowed })
    if ($missing.Count -gt 0) {
        $findings.Add((New-CommandFinding -Severity error -Rule readonly-sem-contrato `
                    -Path "$Source#L1" `
                    -Message "declara ``contract: read-only`` mas ``disallowed-tools`` nao cobre $($missing -join '/') -- ``allowed-tools`` CONCEDE, so ``disallowed-tools`` remove a tool do pool"))
    }

    $conflict = @($allowed | Where-Object { $_ -in $disallowed })
    if ($conflict.Count -gt 0) {
        $findings.Add((New-CommandFinding -Severity error -Rule readonly-contrato-contraditorio `
                    -Path "$Source#L1" `
                    -Message "``allowed-tools`` concede $($conflict -join '/'), que ``disallowed-tools`` remove -- contrato ambiguo"))
    }

    return $findings.ToArray()
}

function Get-MachineEffectFindings {
    <#
    .SYNOPSIS
        Command que se declara de efeito na MAQUINA (`metadata.effect: machine`) nao pode ficar
        model-invocable: precisa de `disable-model-invocation: true`, para que so o humano dispare.
    .DESCRIPTION
        O CRITERIO, e ele vem da propria casa, nao de fora. `commands/build.md` e `commands/iterate.md`
        ja definem **outward** numa tabela de "nao delegue": `git push`/PR/release/deploy. A doc da
        plataforma diz a mesma coisa do campo -- "Use this for workflows with side effects or that
        you want to control timing, like /commit, /deploy [...] You don't want Claude deciding to
        deploy because your code looks ready." E o `AGENTS.md` deste repositorio ja aplica a regra ao
        instalador: "Nao rode -ExtraPlugins/-WithSemanticKb sem perguntar antes -- opt-in e opt-in".

        ESCOPO MEDIDO (2026-08-16, nos 30 commands): so DOIS escrevem fora do repo do projeto --
        `/supplements` (instala em `~/.claude`, escopo `user` e o PADRAO, baixando de marketplace de
        terceiro) e `/update-skills` (escreve em `~/.claude/skills/`). Os demais escrevem dentro do
        repo, onde `git diff` reverte. `/doctor` LE `~/.claude` e nao escreve; `/skill-gap` gera em
        `.claude/skills/` (escopo projeto).

        POR QUE ISTO NAO CEGA O AGENTE: o `CLAUDE.md` do scaffold lista os **30** commands numa
        tabela always-on (bloco `sync-context:start:commands`). Com `disable-model-invocation`, o
        agente continua sabendo que o command existe e para que serve -- o que sai e a capacidade de
        dispara-lo sozinho, mais o custo do descriptor. Sem essa tabela o campo seria bem mais caro.
    .OUTPUTS  [pscustomobject[]] (vazio = conforme, ou command sem efeito de maquina declarado)
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)][AllowEmptyString()][string]$Text,
    ...
    $fm = Get-FrontmatterBlock -Text $Text
    if (-not $fm) { return $findings.ToArray() }
    if ($fm -notmatch '(?m)^\s+effect:\s*["'']?machine["'']?\s*$') { return $findings.ToArray() }

    # YAML aceita yes/no/on/off/1/0 alem de true/false, em qualquer caixa (doc da plataforma).
    $m = [regex]::Match($fm, '(?mi)^disable-model-invocation:\s*["'']?(\w+)["'']?\s*$')
    $on = $m.Success -and $m.Groups[1].Value.ToLowerInvariant() -in @('true', 'yes', 'on', '1')

    if (-not $on) {
        $findings.Add((New-CommandFinding -Severity error -Rule outward-model-invocable `
                    -Path "$Source#L1" `
                    -Message 'declara `effect: machine` (escreve fora do repo do projeto) mas segue model-invocable — exige `disable-model-invocation: true`, senao o agente pode instalar/atualizar na maquina por conta propria'))
    }
...
function Get-CommandLintFindings {
    ...
    foreach ($f in (Get-ReadOnlyContractFindings -Text $Text -Source $Source)) { $findings.Add($f) }
    foreach ($f in (Get-MachineEffectFindings -Text $Text -Source $Source)) { $findings.Add($f) }
    return $findings.ToArray()
}
