...
$script:SddNextCommand = @{ brainstorm = '/define'; define = '/spec'; design = '/build'; build = '/ship' }
...
# --- PURA: qual feature ARQUIVADA "reivindica" um BUILD_REPORT ainda em reports/ ---------------
function Resolve-ReportOwnerKey {
    <#
    .SYNOPSIS
        Devolve a chave da feature JÁ ARQUIVADA (archive/<F>/SHIPPED_*.md) dona de um
        BUILD_REPORT_<name> que continua em reports/, ou '' se nenhuma reivindica.

    .DESCRIPTION
        Entrega em levas gera N relatórios da MESMA feature (`F`, `F_LEVA2`, …). O casamento é por
        PREFIXO DE SEGMENTOS (split em `[_-]`), do mais longo para o mais curto — nunca por
        substring: com `archive/AUTH/` shipado, o relatório `AUTHZ` (outra feature, um segmento só)
        NÃO é reivindicado, enquanto `AUTH_LEVA2` é.

        Direcional de propósito: só uma feature ARQUIVADA reivindica um relatório, nunca o
        contrário. ConvertTo-SlugKey continua intacto — o rigor dele é o que força o erro para o
        lado seguro (falso positivo = ruído; falso negativo = trabalho some do radar).
    .OUTPUTS [string] chave da feature dona, ou '' (nenhuma).
    #>
    param(
        [Parameter(Mandatory)][AllowEmptyString()][string]$ReportName,
        [Parameter(Mandatory)][AllowEmptyCollection()][string[]]$ShippedKeys
    )
    if ([string]::IsNullOrWhiteSpace($ReportName)) { return '' }
    $segments = @($ReportName -split '[_-]+' | Where-Object { $_ })
    for ($n = $segments.Count; $n -ge 1; $n--) {
        $candidate = ConvertTo-SlugKey (($segments[0..($n - 1)]) -join '_')
        if ($ShippedKeys -contains $candidate) { return $candidate }
    }
    return ''
}

# --- I/O leitura: BUILD_REPORT órfão — feature já arquivada, relatório ainda em reports/ --------
function Get-OrphanBuildReports {
    <#
    .SYNOPSIS
        Relatórios em .claude/sdd/reports/ cuja feature JÁ tem archive/<F>/SHIPPED_*.md.

    .DESCRIPTION
        A invariante do /ship (passo 3): ao encerrar, os artefatos SAEM de reports/ e vão para
        archive/<FEATURE>/. Relatório que ficou para trás faz o /status reportar build aberto numa
        feature encerrada — foi o defeito observado em uso real (2026-07-23). Esta é a FONTE ÚNICA
        da detecção: o painel (Format-StatusReport) e o lint (sdd-lint.ps1) leem daqui.
    .OUTPUTS [pscustomobject[]] { Feature; FeatureKey; Report; Path } — vazio = invariante intacta.
    #>
    [CmdletBinding()]
    ...
    if (-not (Test-Path -LiteralPath $reportsDir -PathType Container)) { return @() }

    $shipped = @{}   # chave de slug -> nome real do diretório de archive
    foreach ($d in (Get-ChildItem -LiteralPath $archiveDir -Directory -ErrorAction SilentlyContinue)) {
        ...
        if ($hasShipped) { $shipped[(ConvertTo-SlugKey $d.Name)] = $d.Name }
    }
    ...
    foreach ($f in (Get-ChildItem -LiteralPath $reportsDir -Filter 'BUILD_REPORT_*.md' -File -ErrorAction SilentlyContinue)) {
        ...
        $owner = Resolve-ReportOwnerKey -ReportName $name -ShippedKeys @($shipped.Keys)
        if (-not $owner) { continue }
        $result.Add([pscustomobject]@{
                Feature    = $shipped[$owner]
                FeatureKey = $owner
                Report     = $f.Name
                Path       = ".claude/sdd/reports/$($f.Name)"
            })
    }
    return @($result | Sort-Object Report)
}
...
function Get-SddFeatureStatus {
    <#
    .OUTPUTS [pscustomobject[]] { Feature; Phase; Shipped; OrphanReports } — uma por feature observada.
    .NOTES   OrphanReports: relatórios que o /ship deixou em reports/ (ver Get-OrphanBuildReports).
             Eles NÃO elevam a fase — uma feature encerrada não volta a "build" porque um arquivo
             ficou no lugar errado; o painel os mostra como órfãos e o sdd-lint os reprova.
    #>
    [CmdletBinding()]
    param([Parameter(Mandatory)][string]$Root)
    ...
    # Órfãos primeiro: são a exceção que retira o relatório da contagem de "build em andamento".
    $orphans = @()
    try { $orphans = @(Get-OrphanBuildReports -Root $Root) } catch { Write-Verbose "órfãos indisponíveis: $($_.Exception.Message)" }
    $orphanFiles = @($orphans | ForEach-Object { $_.Report })
    ...
    if (Test-Path -LiteralPath $reportsDir -PathType Container) {
        foreach ($f in (Get-ChildItem -LiteralPath $reportsDir -Filter 'BUILD_REPORT_*.md' -File -ErrorAction SilentlyContinue)) {
            if ($orphanFiles -contains $f.Name) { continue }   # feature já arquivada -> não é build aberto
            $name = $f.BaseName -replace '^BUILD_REPORT_', ''
    ...
    foreach ($g in ($obs | Group-Object Key)) {
        ...
        $orph = @($orphans | Where-Object { $_.FeatureKey -eq $g.Name } | ForEach-Object { $_.Report })
        $result.Add([pscustomobject]@{ Feature = $name; Phase = $phase; Shipped = $shipped; OrphanReports = $orph })
    }
...
# --- PURA: contagem do backlog a partir do TEXTO (formato do templates/project-scaffold/BACKLOG.md)
function Measure-BacklogText {
    <#
    .OUTPUTS [pscustomobject] { Open; Decided; Done; Placeholder }
    .NOTES  PURA (não toca disco) para o parser ser testável sem fixture.
            Contrato mínimo do arquivo: itens `- [ ]` (aberto) e `- [x]` (fechado), sob os headings
            `## Aberto`, `## Decidido: NÃO fazer` e `## Concluído`. A seção decide o que o `- [x]`
            SIGNIFICA — "decidi não fazer" e "fiz" são as duas coisas que o backlog mais precisa
            distinguir, e um checkbox sozinho não as distingue.
            `Placeholder` conta os itens que ainda são o exemplo do template (`<...>` no corpo) —
            eles NÃO entram em Open: um scaffold recém-criado não tem trabalho represado.
    #>
    [CmdletBinding()]
    param([Parameter(Mandatory)][AllowEmptyString()][string]$Text)

    $open = 0; $decided = 0; $done = 0; $placeholder = 0
    $section = ''
    foreach ($line in ($Text -split "`r?`n")) {
        if ($line -match '^\s*##\s+(.+?)\s*$') {
            $h = $Matches[1]
            # pt-BR e ingles por UNIAO: os tres pares sao headings, nao corpo livre, e nenhum termo
            # EN aqui tem ocorrencia no corpus pt-BR (medido 2026-08-28). O modo de falha que isto
            # fecha era SILENCIOSO: um BACKLOG em ingles devolvia Open=0/Decided=0/Done=0 sem um
            # unico finding -- o `/status` reportava fila vazia com a fila cheia.
            $section =
                if     ($h -match '(?i)^(Aberto|Open)\b')                          { 'open' }
                elseif ($h -match "(?i)N[ÃA]O\s+fazer|won'?t\s+do|will\s+not\s+do") { 'decided' }
                elseif ($h -match '(?i)^(Conclu|Done|Closed)')                      { 'done' }
                else                                                               { '' }
            continue
        }
        if ($line -notmatch '^\s*-\s*\[( |x|X)\]') { continue }
        $checked = $Matches[1] -ne ' '
        $isPlaceholder = $line -match '<[^>]+>'
        switch ($section) {
            'open'    { if ($isPlaceholder) { $placeholder++ } elseif (-not $checked) { $open++ } }
            'decided' { if (-not $isPlaceholder) { $decided++ } }
            'done'    { if (-not $isPlaceholder) { $done++ } }
        }
    }
    return [pscustomobject]@{ Open = $open; Decided = $decided; Done = $done; Placeholder = $placeholder }
}

# --- I/O leitura: fila de trabalho do projeto (BACKLOG.md na raiz) -----------------------------
function Get-BacklogStatus {
    <#
    .OUTPUTS [pscustomobject] { Available; Path; Open; Decided; Done; Placeholder }
    .NOTES  Fail-safe: sem BACKLOG.md -> Available=$false (projeto que não usa a fila não "reprova").
    #>
    [CmdletBinding()]
    param([Parameter(Mandatory)][string]$Root)

    $na = [pscustomobject]@{ Available = $false; Path = ''; Open = 0; Decided = 0; Done = 0; Placeholder = 0 }
    $path = Join-Path $Root 'BACKLOG.md'
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { return $na }

    $text = ''
    try { $text = Get-Content -LiteralPath $path -Raw -ErrorAction Stop } catch { return $na }
    $m = Measure-BacklogText -Text $text
    return [pscustomobject]@{
        Available = $true; Path = 'BACKLOG.md'
        Open = $m.Open; Decided = $m.Decided; Done = $m.Done; Placeholder = $m.Placeholder
    }
}
...
function Get-NextStep {
    ...
    .NOTES  Prioridade: setup > curadoria incompleta > feature aberta > inbox > backlog > staleness > nova feature.
    #>
    [CmdletBinding()]
    ...
    # 5) backlog com item aberto — a fila do projeto vem ANTES do "tudo em dia": trabalho já
    #    triado e com critério de entrada respondido é a próxima feature, não uma ideia nova.
    $bk = Get-PropOrNull $Report 'Backlog'
    if ($bk -and [bool](Get-PropOrNull $bk 'Available') -and [int](Get-PropOrNull $bk 'Open') -gt 0) {
        $n = [int]$bk.Open
        return [pscustomobject]@{ Command = '/define'; Reason = "backlog com $n item(ns) aberto(s) -- pegue o topo (ou /dev p/ tarefa pequena)" }
    }
    # 6) staleness da curadoria
...
function Get-StatusReport {
    <#
    .OUTPUTS [pscustomobject] { Root; Curation; Features; Inbox; Backlog; Git; Memory; Staleness; Peers; NextStep }
    ...
    $backlog = $null
    try { $backlog = Get-BacklogStatus -Root $Root } catch { Write-Verbose "backlog indisponível: $($_.Exception.Message)" }
    ...
    $report = [pscustomobject]@{
        Root = $Root; Curation = $curation; Features = $features; Inbox = $inbox; Backlog = $backlog
...
function Format-StatusReport {
    ...
    # Relatório que o /ship deixou em reports/ com a feature já arquivada (invariante do passo 3).
    $orphFiles = @($feats | ForEach-Object { @(Get-PropOrNull $_ 'OrphanReports') } | Where-Object { $_ })
    if ($orphFiles.Count -gt 0) {
        $lines.Add(("              orfaos: {0} relatorio(s) em reports/ de feature ja arquivada" -f $orphFiles.Count))
        foreach ($f in $feats) {
            foreach ($o in @(Get-PropOrNull $f 'OrphanReports')) {
                if ($o) { $lines.Add("              - $o -> mova p/ .claude/sdd/archive/$($f.Feature)/ (/ship passo 3)") }
            }
        }
    }
    ...
    # --- Backlog (fila de trabalho do projeto; some quando o projeto não usa BACKLOG.md) ---
    $bk = Get-PropOrNull $Report 'Backlog'
    if ($bk -and $bk.Available) {
        if ($bk.Open -eq 0 -and $bk.Placeholder -gt 0) {
            $lines.Add('  Backlog   : so o exemplo do template (nenhum item real ainda)')
        }
        else {
            $lines.Add(("  Backlog   : {0} aberto(s) | {1} decidido(s) NAO fazer | {2} concluido(s)" -f `
                        $bk.Open, $bk.Decided, $bk.Done))
        }
    }
