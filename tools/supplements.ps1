...
function Get-SupplementPlan {
    ...
    param(
        ...
        [bool]$Disabled = $false          # plugin/add ganha um 3º passo: `plugin disable`
    )
    ...
    # O `install` não tem flag para nascer desabilitado (CLI 2.1.269, `--help`): desligar é um 3º
    # passo. Desabilitado, o plugin fica no disco e não carrega skill, hook nem MCP — o usuário o
    # liga pelo `/plugin` quando for usar. Motivo: com os 8 temas, ~10,1k tok always-on nos 10
    # plugins medidos, mais 1 MCP e 6 hooks (2026-09-11).
    if ($Disabled) { $steps += , @('plugin', 'disable', "$($Spec.Name)@$($Spec.Id)", '--scope', $Scope) }
    $estado = if ($Disabled) { ', desabilitado' } else { '' }
    return [pscustomobject]@{
        Action = 'add'; Route = 'plugin'; Steps = $steps
        Reason = "instalar $($Spec.Name) [$Scope$estado] — $($Spec.Reason)"
    }
}
...
function Invoke-SupplementsSetup {
    ...
    .PARAMETER Disabled
        Plugin recém-instalado nasce desabilitado (`claude plugin disable` depois do install). É o
        caminho do onboarding: instala o repertório sem pôr o custo dele em toda sessão. Não mexe
        em plugin que já estava instalado nem em Type=skill (skill não tem estado de habilitação).
    #>
    [CmdletBinding()]
    param(
    ...
    $desabilitados = [System.Collections.Generic.List[string]]::new()
    ...
    foreach ($spec in $catalog) {
        ...
        $plan = Get-SupplementPlan -Spec $spec -ClaudePresent $claudePresent -BaselinePresent $baselinePresent -AlreadyInstalled $already -Scope $Scope -Disabled $Disabled.IsPresent
            ...
            'add' {
                if ($plan.Route -eq 'plugin') {
                    ...
                    elseif ($DryRun) {
                            ...
                            foreach ($step in $plan.Steps) {
                                try {
                                    ...
                                    if ($LASTEXITCODE -ne 0) {
                                        # Falhar no disable é o único caso em que o plugin FICOU instalado — e habilitado.
                                        $dica = if ($step[1] -eq 'disable') { 'o plugin ficou HABILITADO; desabilite pelo /plugin' } else { 'instale manualmente depois' }
                                        Write-Step WARN "suplemento: 'claude $($step -join ' ')' retornou $LASTEXITCODE — $dica"
                                        $ok = $false; break
                                    }
                                }
                                catch {
                        ...
                        if ($ok) {
                            $estado = if ($Disabled) { ', desabilitado' } else { '' }
                            Write-Step OK "suplemento instalado: $($spec.Name) [$($spec.Theme)] ($Scope scope$estado)"
                            $Summary.Installed++
                            if ($Disabled) { $desabilitados.Add("$($spec.Name)@$($spec.Id)") }
                        }
                        else { $Summary.Warn++ }
                    }
                }
    ...
    # O instalador termina dizendo onde está o interruptor — sem isto, "desabilitado" parece "não instalou".
    if ($desabilitados.Count -gt 0) {
        Write-Step INFO "suplementos: $($desabilitados.Count) plugin(s) instalado(s) DESABILITADO(s) — não custam contexto até você ligar"
        Write-Step INFO "  habilite o que for usar: /plugin (aba Installed) ou /plugin enable <plugin>@<marketplace>"
        Write-Step INFO "  plugin com MCP (ex.: huggingface-skills) termina a configuração em /mcp"
    }
}
...
function Get-MarketplaceRoot {
    ...
    [CmdletBinding()]
    param([string]$HomePath = (Get-UserHome))
