...
function Invoke-SupplementsSetup {
    ...
    foreach ($spec in $catalog) {
        # "Já instalado?" (read-only) — plugin via 'claude plugin details'; skill via baseline (n/d no MVP).
        $already = $false
        ...
        $plan = Get-SupplementPlan -Spec $spec -ClaudePresent $claudePresent -AlreadyInstalled $already
            ...
            'add' {
                ...
                elseif ($DryRun) {
                    if ($plan.Route -eq 'plugin') {
                        ...
                        Write-Step DRY "baseline install: skill $($spec.Name)"
                    }
                }
                else {
                    $ok = $true
                    if ($plan.Route -eq 'plugin') {
                        ...
                        # Type=skill: caminho de baseline projetado (Type=skill). Sem skill standalone
                        # vendorizada no MVP, não há Item a montar — avisa e segue (não-bloqueante).
                        Write-Step WARN "suplemento: skill $($spec.Name) sem origem vendorizada no baseline — pulei"
                        $ok = $false
                    }
                    if ($ok) {
