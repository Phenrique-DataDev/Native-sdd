...
function Get-DistributionManifest {
    ...
    [pscustomobject]@{
        ...
        IncludeFiles  = @('README.md', 'VERSION', '.gitattributes', '.gitignore',
            ...
            'docs/assets/native-sdd-banner.png')
...
function Test-PublishGate {
    ...
    if ($PiiHits -gt 0) { $reasons.Add("pii-leak: $PiiHits ocorrência(s) de PII no subset (re-scan da superfície distribuída)") }
...
function Invoke-Publish {
    ...
    try {
        ...
        # 3. SCAN — ref-denylist + PII sobre o subset (defesa-em-profundidade).
        # O check.ps1 (passo VERIFY) já roda o pii-lint, mas com allowlist de extensão e sobre a
        # árvore-fonte; aqui re-escaneamos os arquivos REAIS que serão publicados — TODOS, sem
        # allowlist de extensão (.cypher/sem-extensão entram) e SEM honrar `pii-lint:disable`
        # (rede redundante: um opt-out por arquivo não fura o gate de publicação).
        Write-Host "▶ 3/5 SCAN    (ref-denylist + PII)"
        $refDeny = Read-PiiDenylist -Path (Join-Path $RepoRoot '.claude/ref-denylist.txt')
        $piiDeny = Read-PiiDenylist -Path (Join-Path $RepoRoot '.claude/pii-denylist.txt')
        $refHits = [System.Collections.Generic.List[object]]::new()
        $piiHits = [System.Collections.Generic.List[object]]::new()
        foreach ($rel in $files) {
            ...
            foreach ($h in (Get-PiiFindings -Text $text -Source $rel -Denylist $piiDeny)) { $piiHits.Add($h) }
        }
        $steps.Add("SCAN: $($refHits.Count) ref(s) externa(s), $($piiHits.Count) PII")
        ...
        $gate = Test-PublishGate -ChecksOk $checksOk -SubsetViolations $violations.Count `
            -RefHits $refHits.Count -PiiHits $piiHits.Count -TagExists $tagExists -Force:$Force
        if (-not $gate.Passed) {
        ...
        try {
            if (-not $exists) {
                gh repo create $mirrorQ --public --source . --push --description 'Scaffold auto-otimizável para rodar Spec-Driven Development com Claude Code — do onboarding da máquina à execução do projeto, com qualidade verificável.' 2>&1 | Write-Host
        ...
        # FAIL-FAST honesto: se o push do main foi rejeitado (ex.: ruleset "Block force pushes"),
        # NÃO afirme sucesso nem crie o release — o espelho ficaria com main stale e release novo.
        if (-not $pushOk) {
            Write-Host ""
            Write-Host "❌ PUBLISH incompleto: 'git push --force' para o main de $mirrorQ foi REJEITADO"
            Write-Host "   (provável ruleset 'Block force pushes'). O main do espelho NÃO foi atualizado."
            Write-Host "   → libere o force-push em Settings→Rules (desative o ruleset ou adicione-se como bypass) e rode de novo com -Force."
            $steps.Add('PUBLISH: push REJEITADO — main não atualizado')
            return [pscustomobject]@{ Published = $false; Mirror = $Mirror; Tag = $tag; SubsetCount = $files.Count; Gate = $gate }
        }

        gh repo edit $mirrorQ --template 2>&1 | Write-Host
        # gh release create não sobrescreve: se a tag já tem release e veio -Force, recria (mantém a tag git).
        if ($tagExists -and $Force) { gh release delete $tag --repo $mirrorQ --yes 2>&1 | Write-Host }
        ...
        Write-Host ""
        Write-Host "✅ publicado: $Mirror @ $tag ($($files.Count) arquivos · main + release)"
