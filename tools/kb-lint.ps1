...
function Get-KbInventory {
    ...
    # Ignora auxiliares: arquivos cujo NOME começa com '_' (_ABOUT.md, _TEMPLATE.md, _index.yaml)
    # E arquivos dentro de uma PASTA auxiliar iniciada por '_' (_waves/, _reflections/, _lessons/ —
    # planos/ledgers do /train-kb, /reflect, /learn). Sem isso, esses .md entram como "entrada KB
    # inválida" e poluem /status, curation-nudge e o passo de inventário do próprio /reflect.
...
function Invoke-KbLint {
    ...
    .SYNOPSIS
        ...
        Os erros de frontmatter por-entrada seguem em Get-KbInventory (Valid/Errors); aqui é a camada
        de grafo, à parte (espelha Invoke-AgentLint). Auxiliares _*.md e arquivos sob pasta _*/ são
        ignorados (mesma regra do Get-KbInventory).
    .OUTPUTS   finding[]
    #>
    [CmdletBinding()]
    param([Parameter(Mandatory)][string]$Dir)
