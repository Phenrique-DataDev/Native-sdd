...
# --- FONTE ÚNICA da conversão chars -> tokens (todo o repo consome ESTA) -----------------------
# CALIBRADO (2026-07-13) contra o /context real de uma sessão do Claude Code, não chutado:
#
#   arquivo            chars     /context (tokens)   chars/token
#   max-mode.md        18.220         8.100             2,25
#   agent-routing.md   16.100         7.300             2,21
#   AGENTS.md          10.080         4.600             2,19
#
# O valor clássico de ~4 chars/token vale p/ PROSA EM INGLÊS. O conteúdo deste projeto (português
# acentuado + markdown denso: tabelas, backticks, emojis, listas) tokeniza MUITO pior. Com 4, a
# estimativa MENTE PARA BAIXO em ~1,8x — e um orçamento que mente para baixo absolve o que deveria
# sinalizar (foi o que aconteceu no G8: o retrato inocentou as rules que eram 69% do always-on).
#
# Vive AQUI, e não em cada script, porque "quanto custa um char" é UM valor: duplicá-lo é como o
# drift nasce (mesmo precedente do Get-KbEntryFile, v0.8.7 — a cópia derivada é que envenena).
# Consumidores: Format-KbBudgetAdvisory (aqui) e tools/rules-budget.ps1 (dot-source).
$script:CharsPerToken = 2.2

function ConvertTo-KbTokens {
    <#
    .SYNOPSIS
        Chars -> ~tokens (teto). FONTE ÚNICA da conversão no repo — ninguém mais divide por uma
        constante própria. Pura; nunca lança. Continua sendo ESTIMATIVA: o número exato só o
        /context da sessão dá.
    #>
    [CmdletBinding()]
    param([int]$Chars)
    if ($Chars -le 0) { return 0 }
    return [int][math]::Ceiling($Chars / $script:CharsPerToken)
}

# Orçamento de tamanho por entrada (ADVISORY — informativo, nunca impeditivo). Conta o CORPO em
# chars, excluindo frontmatter e fenced code. Generoso de propósito (sinaliza só o que destoa).
# Ver DESIGN_KB_CHAR_LIMIT.md (B7).
#
# ATENÇÃO — os TETOS EM CHARS abaixo NÃO mudaram; o que mudou é o rótulo em tokens, que era falso.
# Com a calibragem real (2,2), um `runbook` no teto custa ~14.500 tok, NÃO os "~8.000" que este
# arquivo anunciava. Apertar os tetos (ex.: runbook 32000 -> 17600 p/ voltar a valer ~8k tok) é uma
# DECISÃO DE PRODUTO em aberto — ver o item do BACKLOG. Aqui só se corrigiu a mentira.
$script:KbSizeBudgetDefault = 16000          # ~7.300 tok — concept/pattern/reference
$script:KbSizeBudget = @{
    'quick-reference' = 4800                 # ~2.200 tok — deve ser curto
    'index'           = 4800
    'runbook'         = 32000                # ~14.500 tok — legitimamente longo (mas é MUITO)
    'spec'            = 32000
}
...
function Format-KbBudgetAdvisory {
    ...
    foreach ($e in $over) {
        ...
        $tok = ConvertTo-KbTokens -Chars $e.Size   # fonte única (2,2) — era /4, que subestimava 1,8x
