<#
.SYNOPSIS
    Staleness-lint dos artefatos derivados: acusa quando AGENT_MAP.md, graph.json ou
...
.DESCRIPTION
    ...
    e mapeia cada DriftResult{InSync=$false} → finding `error` `stale-<key>`:
      ...
      - stale-graph      : graph.json diverge (ou está ausente).
      - stale-kb-index   : kb/_index.yaml diverge (ou ausente).
      - stale-docs-index : docs/_index.md diverge (ou ausente).
      - stale-archive-index : .claude/sdd/archive/_index.md diverge (ou ausente).
...
$script:ResyncRuleFor = @{ 'map' = 'stale-map'; 'graph' = 'stale-graph'; 'kb-index' = 'stale-kb-index'; 'docs-index' = 'stale-docs-index'; 'archive-index' = 'stale-archive-index' }
