...
.DESCRIPTION
    ...
    corpo excluindo fenced code blocks) e de "como ler frontmatter". O arquivo inteiro de uma rule é
    o corpo; rule com `paths:` no frontmatter é CONDICIONAL e sai do total (Get-ConditionalRuleInventory).
    ...
    O contexto always-on de uma sessão tem partes de origens diferentes, e o script NÃO as mistura:
      ...
      MEDIDO (vem do disco, este script conta):
        - rules       .claude/rules/*.md          — auto-carregadas inteiras, imposto permanente (exceto
                                                    as com `paths:`, medidas à parte)
...
# --- PURA: rule condicional (frontmatter com `paths:`) -----------------------------------------
function Test-ConditionalRule {
    <#
    .SYNOPSIS
        $true se a rule declara `paths:` no frontmatter. O harness só a carrega quando o modelo lê um
        arquivo que casa o glob — medido em 2026-09-24 (CLI 2.1.282): sentinela condicional ausente
        em 0/2 sessões sem leitura e presente em 3/3 depois de um `Read` casado, com o `stream-json`
        mostrando que o único arquivo lido foi o casado.
    #>
    [CmdletBinding()]
    param([Parameter(Mandatory)][string]$Path)

    $fm = Read-KbFrontmatter -Path $Path
    return ($null -ne $fm -and $fm.Contains('paths'))
}

function Get-ConditionalRuleInventory {
    <#
    .SYNOPSIS
        Mede as rules de RulesDir com `paths:` — medidas, FORA do total always-on (mesmo tratamento
        das posturas: o ganho fica auditável e a regressão, visível).
    .OUTPUTS
        [pscustomobject[]] @{ Path; Name; Kind('rule-conditional'); Chars; Tokens }
    #>
    [CmdletBinding()]
    param([string]$RulesDir)

    if (-not $RulesDir -or -not (Test-Path -LiteralPath $RulesDir -PathType Container)) { return @() }

    $items = [System.Collections.Generic.List[object]]::new()
    foreach ($f in (Get-ChildItem -LiteralPath $RulesDir -Filter '*.md' -File | Sort-Object Name)) {
        if (-not (Test-ConditionalRule -Path $f.FullName)) { continue }
        $chars = Measure-KbContentSize -BodyLines @(Get-Content -LiteralPath $f.FullName -ErrorAction SilentlyContinue)
        $items.Add([pscustomobject]@{
            Path   = $f.FullName
            Name   = $f.Name
            Kind   = 'rule-conditional'
            Chars  = $chars
...
function Get-AlwaysOnInventory {
    ...
    if (Test-Path -LiteralPath $RulesDir -PathType Container) {
        foreach ($f in (Get-ChildItem -LiteralPath $RulesDir -Filter '*.md' -File | Sort-Object Name)) {
            # Rule com `paths:` não é always-on: vai para Get-ConditionalRuleInventory.
            if (Test-ConditionalRule -Path $f.FullName) { continue }
...
function Measure-DescriptorSurface {
    ...
    # `disable-model-invocation: true` tira o item do contexto do modelo -- a doc oficial e
    # explicita: "removes the skill from Claude's context entirely" / "Description not in context,
    # full skill loads when you invoke". Logo o descriptor NAO e superficie always-on. Sem este
    # predicado o retrato contaria como custo fixo algo que o harness nao injeta, e o instrumento
    # mentiria justamente sobre a economia que o campo existe para produzir.
    #
    # YAML aceita yes/no/on/off/1/0 em qualquer caixa alem de true/false (doc, `skills`); aceitamos
    # o mesmo conjunto -- ler so 'true' daria falso-negativo em frontmatter valido.
    $invocable = $true
    if ($fm.Contains('disable-model-invocation')) {
        $flag = ([string]$fm['disable-model-invocation']).Trim().Trim('"').Trim("'").ToLowerInvariant()
        if ($flag -in @('true', 'yes', 'on', '1')) { $invocable = $false }
    }
...
function Get-DescriptorInventory {
    ...
    foreach ($f in ($files | Sort-Object)) {
        ...
        $items.Add([pscustomobject]@{
            ...
            ModelInvocable = $d.ModelInvocable
        })
    }
    return $items.ToArray()
}
...
function Get-SessionInventory {
    ...
    .SYNOPSIS
        ...
        caminho de código.
    .OUTPUTS
        [pscustomobject] @{ AlwaysOn; Descriptors; Hidden; Postures; Conditional; Memory; McpServers }
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)][string]$RulesDir,
    ...
    # Descriptors de `disable-model-invocation: true` saem do always-on pelo mesmo motivo das
    # posturas: existem no disco, custam 0 ate alguem invoca-los. Medidos e exibidos a parte --
    # some-los do retrato esconderia o preco que se paga ao invocar; conta-los no total inventaria
    # um custo que o harness nao cobra.
    $all = $desc.ToArray()

    return [pscustomobject]@{
        AlwaysOn    = $alwaysOn
        Descriptors = @($all | Where-Object { $_.ModelInvocable })
        Hidden      = @($all | Where-Object { -not $_.ModelInvocable })
        Postures    = @(Get-PostureInventory -PosturesDir $PosturesDir)
        Conditional = @(Get-ConditionalRuleInventory -RulesDir $RulesDir)
...
function Format-AlwaysOnReport {
    ...
    # 2b) Descriptors FORA do contexto (`disable-model-invocation: true`) — medidos, fora do total.
    # Acesso DEFENSIVO: um inventário montado à mão (teste, chamador externo, versão anterior) não
    # tem a chave, e sob Set-StrictMode a propriedade ausente LANÇA — o relatório inteiro morreria
    # por uma seção opcional. Foi um teste preexistente que pegou isto, não uma revisão.
    # (O `@(...)` externo é obrigatório: um `@()` devolvido de dentro de um `if` é desenrolado e a
    # variável recebe $null — aí `.Count` lança sob StrictMode, trocando um erro por outro.)
    $hidden = @(if ($session.PSObject.Properties.Name -contains 'Hidden') { $session.Hidden })
    if ($hidden.Count -gt 0) {
        $hSub = 0
        foreach ($d in $hidden) { $hSub += [int]$d.Tokens }
        $lines.Add('')
        $lines.Add(('  FORA do contexto do modelo (`disable-model-invocation`) — só você dispara; NÃO somam no always-on  (~{0} tok evitados, {1} itens)' -f $hSub, $hidden.Count))
        $rankedH = @($hidden | Sort-Object @{ e = { [int]$_.Tokens }; Descending = $true }, Name)
        $wH = 0
        foreach ($d in $rankedH) { if ($d.Name.Length -gt $wH) { $wH = $d.Name.Length } }
        foreach ($d in $rankedH) {
            $lines.Add(('    {0}  {1,5} tok' -f ([string]$d.Name).PadRight($wH), [int]$d.Tokens))
        }
    }
    ...
    # 3b) Rules condicionais (`paths:`) — fora do total pelo mesmo motivo das posturas. Mesmo guard
    # do Hidden: inventário montado à mão pode não ter a chave.
    $conditional = @(if ($session.PSObject.Properties.Name -contains 'Conditional') { $session.Conditional })
    if ($conditional.Count -gt 0) {
        $cSub = 0
        foreach ($c in $conditional) { $cSub += [int]$c.Tokens }
        $rankedC = @($conditional | Sort-Object @{ e = { [int]$_.Tokens }; Descending = $true }, Name)
        $wC = 0
        foreach ($c in $rankedC) { if ($c.Name.Length -gt $wC) { $wC = $c.Name.Length } }

        $lines.Add('')
        $lines.Add(('  rules CONDICIONAIS (`paths:`) — 0 tok até o modelo ler arquivo casado; NÃO somam no always-on  (~{0} tok evitados, {1} arquivos)' -f $cSub, $conditional.Count))
        foreach ($c in $rankedC) {
            $lines.Add(('    {0}  {1,6} tok' -f ([string]$c.Name).PadRight($wC), [int]$c.Tokens))
        }
    }
...
function Invoke-RulesBudget {
    ...
    # Resolve em VARIÁVEL e sai cedo quando não resolve. A linha anterior era
    # `$Root = (Resolve-Path ... -SilentlyContinue).Path`: com caminho inválido o Resolve-Path
    # devolve $null, o acesso a .Path lança sob Set-StrictMode, o erro sobe como não-terminante,
    # $Root fica $null, todos os Join-Path seguintes degradam — e o `exit 0` advisory do fim do
    # arquivo esconde o estrago. O resultado medido era um relatório BEM-FORMADO
    # (`always-on MEDIDO: ~1350 tok (1 arquivos)`) para um caminho que não existe.
    #
    # Dói exatamente em quem está tentando responder "quanto estou pagando de contexto?" — a
    # pergunta de que a hipótese "mais barato" depende, e o único instrumento que o produto oferece
    # para ela. O auditor externo caiu nisso no meio da própria auditoria.
    $resolved = Resolve-Path -LiteralPath $Root -ErrorAction SilentlyContinue
    if (-not $resolved) {
        Write-Host "rules-budget: -Root nao resolve: $Root"
        return
    }
    $Root = $resolved.Path
