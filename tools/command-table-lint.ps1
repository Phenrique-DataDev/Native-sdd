...
.DESCRIPTION
    ...
    O que é AVISO (warn, NÃO bloqueia):
      - `table-row-too-long` : a célula de propósito passa do orçamento de ~12 palavras que o
        próprio `/sync-context` prescreve para este bloco (Passo 3, "Orçamento destes blocos").
        A tabela é always-on e, no Claude Code, o harness JÁ injeta nome+description de todo
        command — linha longa aqui é a mesma informação cobrada duas vezes. Warn, não error,
        porque o orçamento é "~12" (aproximado) e reprovar um texto por uma palavra seria
        arbitrário; o aviso existe porque regenerar "caprichado" desfaz o enxugamento em
        silêncio, e foi exatamente o que aconteceu com este bloco (14 de 30 linhas acima do
        orçamento em 2026-08-10, enquanto o bloco irmão de `rules` já respeitava).

    O que NÃO é violação:
      - ordem das linhas / agrupamento / coluna de fase — só o CONJUNTO de nomes importa.

    Severidade: `error` (conjunto de nomes) + `warn` (orçamento de linha). Só error bloqueia o CI.
#>
...
function Get-CommandTableRowBudget {
    <#
    .SYNOPSIS  Para cada linha de command da região marcada, devolve nome + nº de palavras da
               célula de PROPÓSITO. PURA (só texto).
    .DESCRIPTION
        A célula de propósito é a ÚLTIMA célula não-vazia da linha — assim a contagem funciona
        tanto no formato de 2 colunas (nome | propósito) quanto no antigo de 3 (nome | fase |
        propósito), sem que mudar o layout da tabela vire falso-positivo.
    ...
    foreach ($line in ($inner -split "\r?\n")) {
        ...
        $cells = @($line.Trim().Trim('|') -split '\|' | ForEach-Object { $_.Trim() } | Where-Object { $_ })
        $purpose = if ($cells.Count -ge 2) { $cells[-1] } else { '' }
        $words = @($purpose -split '\s+' | Where-Object { $_ }).Count
...
function Get-CommandTableFindings {
    ...
    foreach ($row in (Get-CommandTableRowBudget -Text $Text)) {
        if ($row.Words -le $MaxWords) { continue }
        $findings.Add((New-CommandTableFinding -Severity warn -Rule table-row-too-long -Path "$Source#/" `
                    -Message "/$($row.Name): $($row.Words) palavras (orçamento ~$MaxWords) — a tabela é always-on e o harness já injeta a description; encurte em vez de caprichar"))
    }
    return $findings.ToArray()
}
