...
<p align="center">
  ...
  <img alt="Windows-first" src="https://img.shields.io/badge/platform-Windows--first-0078D6?logo=windows&logoColor=white">
...
4. CURADORIA   /init                   → /audit-agents + /train-kb + /sync-context especializam
...
guardas (`secret-guard`, `main-push-guard`, `destructive-guard`, managed policy) seguem **ativos** e
...
```
.
├── onboarding/     Instalador automático e criação de projetos (Windows-first)
│   ├── install.ps1         prepara a máquina + ~/.claude (CLIs, settings, statusline)
│   ├── new-project.ps1     cria/atualiza um projeto com o scaffold (New-SddProject / nsp)
│   ├── windows/            impl. real: apply, install-clis, install-mcp (context7), install-plugins (opt-in), lib.ps1 (puras)
│   ├── macos/ · linux/     stubs conscientes (apply.sh) — Windows é a impl. real
│   └── tests/              Pester (lib, merge .json, backup, push-guard, secret-scan)
...
│   ├── global-claude/      config pessoal de nível usuário (~/.claude: CLAUDE.md, settings, statusline, hooks: push-guard + secret-guard)
...
│   ├── doc-lint · doubt-lint · standards-lint · hooks-lint · pii-lint · rules-budget   docs / rules / hooks / PII / orçamento de contexto
...
│   ├── graph-export · publish · simulate     property-graph (neo4j) · distribuição (espelho público) · simulador de stack
...
| **Agents** | **8 experts de papel** universais (ver [Agentes](#agentes--agents)); os de **domínio** surgem na curadoria | `.claude/agents/` |
...
| **Hooks** | `curation-nudge` (staleness, read-only) + guards de segurança (`secret-guard`, `main-push-guard`, `destructive-guard`) | `.claude/hooks/` |
...
O scaffold traz **8 subagents de papel/disciplina** — **universais** (agnósticos de stack), não de
...
8 papéis universais valem para qualquer projeto.
...
6. **Segurança em camadas, sem barrar.** A rede é **hook-based**, não prompt-based: git **pre-commit**
   ...
   **main-push-guard** (confirma push de código na default) e **managed policy** (deny inviolável:
...
- **Windows + PowerShell 7+** (impl. real; macOS/Linux têm stubs conscientes).
- **Claude Code** (CLI/desktop). O `install.ps1` instala as CLIs esperadas (`git`, `gh`, `node`,
  `python`, `uv`, `jq`, `yq`, `ripgrep`) via winget.
