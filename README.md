...
> **Sem clonar / No clone needed** — um comando baixa o conteúdo de `main` e delega ao instalador
> (alternativa ao fluxo clonado abaixo, que continua valendo):
>
> ```powershell
...
>
> ```bash
> # Linux (precisa de bash + curl + tar; o script avisa e sugere o comando se algo faltar)
> curl -fsSL https://raw.githubusercontent.com/Phenrique-DataDev/Native-sdd/main/onboarding/bootstrap.sh | bash
> ```
>
> Detalhes, flags (`-Check`/`--check`) e limitações conhecidas: [`docs/USO.md`](docs/USO.md#1-preparar-a-máquina--installps1-windows--installsh-linux).
...
│   ├── windows/            impl. real: apply, install-clis, install-mcp (context7), install-plugins (suplementos opt-in, -Themes), install-local-ai (MCP local via Ollama, opt-in), install-semantic-kb (MCP de busca semântica, opt-in), lib.ps1 (puras)
│   ├── local-ai/           server MCP versionado (modelo local via Ollama) + bench + installer .sh (macOS/Linux) — opt-in via install.ps1 -WithLocalAi
│   ├── semantic-kb/        server MCP de busca semântica (Ollama + sqlite-vec) + testes pytest + installer .sh — opt-in via install.ps1 -WithSemanticKb
...
│   └── supplements/skills/ skills autorais do repertório de suplementos (opt-in, só via /supplements — NUNCA espelhada por inteiro como global-claude/)
...
> **[`AGENTS.md`](AGENTS.md)/[`CLAUDE.md`](CLAUDE.md) da raiz** (contrato de onboarding: o que
> fazer quando alguém pede "setup completo" — inclui **sempre oferecer** os opcionais do instalador,
> nunca pular em silêncio). As pastas e arquivos de **desenvolvimento** — `features/`, `.claude/`
> (raiz), `tools/tests/`, `tools/publish.ps1`, `onboarding/tests/` e `CHANGELOG.md` — **não** vêm
> na cópia.
...
| **Skills** | Capacidades sob demanda — **internas** (autoradas pelo próprio scaffold, shipadas por padrão, ex. `decision-preview`, `page-to-markdown`) ou **de terceiro** via `/supplements` (opt-in, user-scoped) | `.claude/skills/` |
| **Hooks** | `curation-nudge` (staleness, read-only) + `peer-heartbeat` (presença p/ `/peers`) + `agent-graph-context` (role+connects_to no subagente) + `complementary-repo-guard` (ask se Write/Edit mirar um repo complementar) + guards de segurança globais (`secret-guard`, `destructive-guard`) | `.claude/hooks/` |
...
| `security-reviewer` | security | **Profundidade** em segurança: segredos, injeção, authn/authz, deps; aponta p/ a skill `security-guidance` quando instalada |
...
| `documenter` | documentation | Documentar/registrar em `docs/` (humano×LLM, **fora da KB**), nunca-destrutivo; aponta p/ as skills `visual-explainer`/`gerador-de-manuais` quando instaladas |
...
| `designer` | design | Criar/revisar/refinar **UI/frontend** — shape→craft→audit (a11y, anti-slop, motion, static-first); aponta p/ a skill `impeccable` quando instalada |
| `tracker` | tracking | Instrumentar/revisar **tráfego e traqueamento** — plano de medição→dedup→consentimento→QA (Consent Mode negado por default, first-party/server-side, dedup por `event_id`, PII hasheada, UTM) |
...
11 papéis universais valem para qualquer projeto.
...
| `/adapt` | Adota projeto **existente** (brownfield): detecta stack + higiene, **absorve** `.claude/` já aplicado (skills/agents/kb/rules/hooks/settings — preserva, nunca sobrescreve) e delega ao `/init` |
...
| `/supplements` | **Repertório curado** de skills/plugins por disciplina (design · reporting · data · security · docs · ai · meta): lista por tema e instala (opt-in, user-scoped) |
...
| `/complementary-repos` | **Repositórios complementares de referência**: registra outros repos (path local/URL git) que o agente consulta **read-only** e adapta (nunca vendoriza); `add`/`list`/`remove` |
...
| Reaproveitar um padrão de outro repositório meu | `/complementary-repos add <path\|url> --reason "..."` |
...
- **semantic-kb MCP** — opcional (`install.ps1 -WithSemanticKb`), independente do local-ai: **busca
  semântica local** (Ollama + `sqlite-vec`) sobre KB/`docs/`/histórico de features — complementa
  `Grep`/`Glob`, nunca substitui; reindexação incremental disparada em pontos do fluxo normal
  (`/train-kb`, `/sync-context`, `/document`, `/ship`), nunca um processo em segundo plano. Exige
  Ollama + `uv`; ver [`onboarding/semantic-kb/`](onboarding/semantic-kb/README.md).
