...
para pular o gate já vem refutada no próprio artefato. As duas seções são uma **convenção**
(não mais verificada por lint automático), aplicada onde há gate real — não em todo
command/agent.
...
| Anti-racionalização | tabela desculpa→realidade + red flags nos gates | `workflow-sdd.md`, `/define`/`/build`/`/ship`/`/review` |
