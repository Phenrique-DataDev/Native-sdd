...
## Orçamento de contexto always-on (G8)
...
rule. Para que esse crescimento não fique invisível, `tools/rules-budget.ps1` **mede** o footprint
always-on (chars→tokens, reusando `Measure-KbContentSize` do B7 — exclui fenced code) e sinaliza
**outlier por-arquivo** / **drift agregado**.

A disciplina é **advisory** — espelha o `kb.char_limit` (B7): **educar ≠ barrar**. O `check.ps1` imprime
o painel a cada run (visibilidade no PR), mas o orçamento **nunca** bloqueia o build (exit 0; não toca o
veredito). Os tetos são **generosos** de propósito (pegam só o que destoa, não espremem rule legítima);
uma rule grande **por mérito** se isenta com o marcador `<!-- context-budget: exempt -->`.
...
|--------|-------|-----------|
| Contexto always-on | orçamento advisory das rules (nunca bloqueia; tetos generosos) | `tools/rules-budget.ps1`, `check.ps1` |
