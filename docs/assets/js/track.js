...
(function () {
  ...
  /**
   * A versão do site que gerou o hit (coluna `rel`, SCHEMA 3 do coletor).
   *
   * Lida do `<meta name="site-version">` em vez de escrita aqui: o HTML é o que já carrega a
   * versão e o que já tem um caso amarrando-a ao `VERSION` do repositório
   * (`tools/site-tests/specs/integridade.spec.mjs`). Uma constante neste arquivo seria uma
   * segunda cópia, e a segunda cópia é a que envelhece — foi assim que a landing passou a afirmar
   * v0.10.58 com o repositório em 0.11.0.
   *
   * Sem o meta, fica string vazia: coluna vazia é um estado que o `?diag=` conta e mostra, e é
  ...
  function buildPayload(name, extra) {
    ...
    return {
      ...
      // --- SCHEMA 3 do coletor (2026-09-20): o que faltava para um BI consumir isto ---
      // `eid` é por EVENTO (o `sid` é por sessão, o `seq` é por carga): é a chave que deixa a
      // ingestão fazer MERGE em vez de INSERT, e sem ela reprocessar uma janela duplica fato.
      eid: idNovo(),
      // `cts` é o instante no relógio do CLIENTE e `tzoff` o offset do fuso dele em minutos a
      // leste de UTC (São Paulo = -180). O coletor carimba o tempo do SERVIDOR; estes dois são o
      // que torna "que horas eram para quem visitou" uma pergunta respondível. Dado de cliente,
      // então ADICIONAL: nada que dependa de confiança pode sair daqui.
  ...
  function bindCopy(btn, getText, doneLabel, eventName) {
    if (!btn) return;
    // innerHTML, não textContent: o botão do quickstart tem a seta em SVG, e restaurar só o texto
    // a apagava depois da primeira cópia (visto em 2026-09-12).
  ...
  // Copia o comando do sistema MARCADO no alternador: cada radio carrega o seu `data-copy`. O do
  // botão é o valor de partida (Windows, o radio que já nasce marcado).
  var copyQsBtn = document.getElementById("copyQs");
  bindCopy(copyQsBtn, function () { var r = document.querySelector(".qs-os-radio:checked"); return (r && r.getAttribute("data-copy")) || copyQsBtn.getAttribute("data-copy") || ""; }, "✓ copiado", "copy_quickstart");
