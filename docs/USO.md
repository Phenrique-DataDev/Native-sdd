...
4. CURADORIA  (em construção — EPIC G)    → especializa agentes/KB ao domínio
5. EXECUTAR   SDD                         → /brainstorm → /define → /design → /build → /ship
```

> Os passos 1–3 e 5 estão prontos. O passo 4 (curadoria automática) é a próxima feature
> (ver [`../features/BACKLOG.md`](../features/BACKLOG.md), EPIC G).

---

## 1. Preparar a máquina — `install.ps1`

Deixa o PC pronto para dev/dados com Claude Code: instala dependências e monta o
`~/.claude` pessoal. **Windows** implementado (macOS/Linux são stubs).

Um PC recém-formatado costuma ter só o **Windows PowerShell 5.1** com `ExecutionPolicy`
...
> **Curadoria (passo 4 do fluxo, EPIC G — em construção):** após o `/setup`, a ideia é
> rodar `/audit-agents` (cura/gera os agentes do domínio) e `/train-kb` (popula a KB). Ainda
> não implementado — acompanhe em [`../features/BACKLOG.md`](../features/BACKLOG.md).
...
| O que vem a seguir | [`../features/BACKLOG.md`](../features/BACKLOG.md) |
