...
| `.\onboarding\install.ps1 -SetDefaultShell` | **Opt-in:** torna o PowerShell 7 o perfil **padrão** do Windows Terminal |
...
> do Windows Terminal (com **backup**, substituição cirúrgica que preserva seus comentários). Não
> bloqueante: qualquer falha vira aviso, nunca aborta a instalação. **Não** mexe na associação de
> arquivos `.ps1` — decisão de segurança, ver a nota no [`BACKLOG`](../features/BACKLOG.md).
...
# Protótipo/MVP: scaffold enxuto (só o núcleo de rules, sem os hooks de processo)
.\onboarding\new-project.ps1 -Path C:\dev\proto -Profile mvp
```

> **`-Profile full` (default) · `mvp`.** O scaffold `full` entrega tudo — ~35k tokens de rules
> always-on + os 4 hooks de processo (`curation-nudge`, `peer-heartbeat`, `agent-graph-context`,
> `complementary-repo-guard`) — desenhado para um projeto que vai viver anos sob SDD. O `mvp`
> entrega só o **núcleo** (`workflow-sdd`, `project-context`, `cli-first`, `tooling`) e **não**
> instala esses hooks: economia medida de **~20k tokens** de contexto always-on, para
> protótipo/MVP onde a cerimônia completa ainda não se paga.
>
