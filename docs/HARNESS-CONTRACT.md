...
> `destructive-guard`. Não instalados por padrão (referência opt-in). Detalhe completo
> em `.claude/sdd/archive/adapter/SHIPPED_2026-07-06.md` (repositório de desenvolvimento).
>
> **Atualização 2026-08-04 — o adapter Codex tem e2e ao vivo, e a limitação de 2026-07-06 caiu.**
> *"Verificação via doc + fixtures sintéticas, não e2e ao vivo (nenhum dos 3 harnesses está
> instalado)"* valeu por ~1 mês; hoje **`codex-cli 0.146.0` está instalado**. Rodado ponta a ponta
> com o Codex dirigindo: `chmod -R 777 ./alvo` → **`hook: PreToolUse Blocked`**, com a razão do
> guard chegando ao modelo; controle (`echo`) passa. **A tabela por-harness abaixo estava certa** —
> o payload real trouxe `hook_event_name: "PreToolUse"`, `tool_name: "Bash"`, `tool_input.command`
> e `cwd`, exatamente como a coluna Codex previu **por doc**, um mês antes.
>
> **O que só o e2e pegou — gate de CONFIANÇA (§Instalação no Codex, abaixo):** copiar o
> `hooks.json`/`config.toml` **não ativa o hook**. Hook não-gerenciado nasce `Untrusted` e **não
> roda, em silêncio**, com o comando executando normalmente — medido: a primeira rodada, seguindo
> os `.example` ao pé da letra, não invocou o hook e o comando passou. **Guard que não dispara é
> pior que guard ausente: a instalação parece feita.** Os dois `.example` foram corrigidos.

> **Atualização 2026-08-04 (2) — a camada de *workflow* também tem caminho para o Codex, e ela não
> passa por hook.** O Codex **não tem slash command definido por usuário** — o `SlashCommand` é enum
> fechado (`codex-rs/tui/src/slash_command.rs`); o ponto de extensão é **skill**:
> `.agents/skills/<nome>/SKILL.md`, descoberto por walk do cwd até a raiz do projeto
> (`codex-rs/core-skills/src/loader.rs`). **`tools/harness-export.ps1` gera** `.agents/skills/` a
> partir de `.claude/commands/` — gerador, não cópia: os 30 SKILL.md não são commitados no scaffold,
> senão viram duplicação a sincronizar à mão. **Medido ao vivo:** as 30 skills aparecem na sessão
> do Codex. **Ver §Exportar o workflow para o Codex.**

## Exportar o workflow para o Codex — `tools/harness-export.ps1`

```powershell
. "$env:SDD_WORKFLOW_HOME/tools/harness-export.ps1"
Export-CodexHarness -ProjectRoot . -DryRun            # mostra o plano
Export-CodexHarness -ProjectRoot .                    # escreve .agents/skills/ e .codex/hooks.json
Export-CodexHarness -ProjectRoot . -InstallAgentRole  # + instala os agentes no CODEX_HOME
```

**Opt-in, e não é chamado por command nenhum** — só interessa a quem dirige o projeto por outro
harness. Reexporte depois de mexer nos commands: a fonte única é o `.claude/commands/`.

| Trava | Por quê |
|---|---|
| **SKILL.md sem BOM** | O parser do Codex faz `content.strip_prefix("---\n")`. **Medido:** com BOM, o frontmatter não é lido e **a skill some da sessão**. É o oposto da convenção deste repo (BOM sempre) — por isso está escrito no código e travado por teste |
| **Bloco de degradação** | Command que usa `Agent`/`Workflow`/`Artifact` gera uma skill com aviso do que **não** existe no Codex, mandando executar o passo em vez de delegar. Melhor degradar declarando do que entregar instrução que o harness não cumpre |
| **Caminho absoluto no `hooks.json`** | O Codex não tem `${CLAUDE_PROJECT_DIR}`. Moveu o projeto, reexporte |

**O que o export NÃO entrega, e por quê (não é esquecimento):**

- **Os hooks de `Write`/`Edit` ficam de fora.** O `curation-nudge` e o `complementary-repo-guard`
  leem `tool_input.file_path`, e **o payload de edição de arquivo do Codex não foi medido** — o
  sandbox de escrita do Windows foi removido do Codex (`-s workspace-write` cai para `read-only`) e
  o caminho alternativo exige flag de bypass. Hook que lê campo que talvez não exista **degrada em
  silêncio**, que é o modo de falha que o gate de confiança já cria uma vez. Só `SessionStart`
  (medido) é emitido.
- **As `.claude/skills/` (2) e as `.claude/rules/` (12).** As rules seguem só pelo sumário de 1
  linha no `AGENTS.md`, como já estava.

## Agent roles — `-InstallAgentRole` (opt-in, escreve FORA do projeto)

> **Atualização 2026-08-05 — os agentes têm porte, e as três afirmações que este documento fazia
> sobre eles caíram na medição.** Eram **11**, não 12 (o 12º arquivo é o `AGENT_MAP.md`, gerado); o
> parser de markdown (`external-agent-migration/src/subagents.rs`) é o formato **legado** — a
> feature flag `external_migration` está `removed` no 0.146.0; e o porte **não** era "quase de
> graça", porque o formato atual é **TOML** e o namespace é **global**.

`Export-CodexHarness -InstallAgentRole` (ou `Export-CodexAgentRole` direto) gera um
`<CODEX_HOME>/agents/<slug-do-projeto>/<agente>.toml` por agente. **Medido no codex-cli 0.146.0:**

| Fato medido | Como | Consequência no gerador |
|---|---|---|
| **Descoberta é global, não por projeto** | `.toml` com campo inválido em `$CODEX_HOME/agents/` → `warning: Ignoring malformed agent role definition` nomeando o arquivo; o **mesmo** arquivo em `<projeto>/.agents/` ou `<projeto>/.codex/agents/` → **silêncio**, inclusive com repo git e `trust_level = "trusted"` | Escreve no `CODEX_HOME`. Gerar dentro do projeto entregaria arquivo **inerte** — o defeito dos `.example` de hook, de novo |
| **Colisão de `name` é silenciosa** | dois roles com `name = "code-reviewer"` em subpastas distintas → o Codex expõe **um só**, sem aviso | `name` sai **prefixado** com o slug do projeto. Sem isso, o 2º projeto scaffolded apagaria os agentes do 1º — e o nosso `explorer` colidiria com o **built-in** `explorer` do próprio Codex |
| **O prefixo só protege se o slug preservar o nome** | `Ação/` e `Aço/` geravam **ambos** `a-o-` até a v0.10.32: o filtro `[^a-z0-9]+` tratava todo diacrítico como separador, e dois projetos distintos do mesmo dono se sobrescreviam — em silêncio, exatamente o que a linha acima existe para impedir | O slug **translitera** antes de filtrar (decomposição Unicode, sem code page). Nome que não folda para ASCII (CJK, cirílico) recebe sufixo do SHA-256 do nome original: ilegível, porém **único** — no namespace global, unicidade é o que não pode faltar |
| **`deny_unknown_fields`** | um campo desconhecido descarta o **role inteiro** | Só `name`, `description`, `sandbox_mode`, `developer_instructions` são emitidos (travado em teste) |
| **`sandbox_mode` ∈ `read-only`/`workspace-write`/`danger-full-access`** | a lista veio da mensagem de erro do Codex ao recusar valor inválido | Agente sem `Edit`/`Write` vira `read-only`; **`tools:` ausente vira `workspace-write`**, porque rebaixar quem escreve o faria falhar em silêncio |
| **Não há equivalente do `tools:`** | `tools = "Read, Grep"` → `invalid type: string [...] expected struct ToolsToml`; o `[tools]` do Codex liga/desliga ferramenta **nativa** (shell, apply_patch, view_image, plan) | O allowlist vira **texto** nas instruções do role, com a degradação declarada |

**Opt-in de propósito:** é o único export que escreve fora do projeto. Sem a flag, `RoleCount = 0`
e nada é criado no `CODEX_HOME`.

**E2E ao vivo (2026-08-05):** os 11 roles do scaffold instalados, listados pelo Codex como
`sdd-e2e-<agente>` sem nenhum warning, e o `sdd-e2e-validator` **usado numa sessão** devolveu a
função escrita no corpo do agente — as `developer_instructions` chegam ao modelo.

**NÃO medido:** se `tools = { ... }` (a struct) dentro de um role surte efeito — campo desconhecido
ali passa em silêncio, então a sonda de erro não decide.

## Instalação no Codex — o passo de confiança (não é opcional)

O Codex gateia hook por **confiança**, não só por configuração
(`codex-rs/hooks/src/engine/discovery.rs`: `hook_trust_status` → `Managed`/`Trusted`/`Modified`/
`Untrusted`; só `Managed` e `Trusted` são ativados, salvo bypass explícito). Depois de copiar o
exemplo, faça **uma** das duas:

| Via | Como | Quando |
|---|---|---|
| **Confiar (recomendada)** | abrir o Codex e rodar **`/hooks`**, revisar e confiar | uso interativo normal |
| **Bypass** | `codex exec --dangerously-bypass-hook-trust …` | automação que já auditou a origem do hook |

> **A confiança é por hash do `command`.** Mudou a linha de comando (path do adapter, flag), o
> status vira `Modified` e o hook **para de rodar** até ser reconfiado — mesma falha silenciosa.
> Confira sempre com `/hooks`.
...
- Brainstorm: `.claude/sdd/archive/adapter/BRAINSTORM_ADAPTER.md`.
- Define/Design/Build/Ship: `.claude/sdd/archive/adapter/`
