...
# COM aviso por systemMessage, 1x/dia (_sdd_ask_degraded).
#
...
# Sai como systemMessage no STDOUT (stderr de hook com exit 0 nao chega ao usuario) -- ver o par em
# secret-guard.sh. Marcador proprio por guard: um guard degradado nao cala o aviso do outro.
_sdd_ask_degraded() {
  local m="${TMPDIR:-/tmp}/.sdd-ask-degraded.destructive.$(date +%Y%m%d 2>/dev/null)"
  [ -e "$m" ] && return 0
  printf '{"systemMessage":"[sdd] aviso: %s ausente - guards interativos (.sh) INATIVOS (passthrough: destrutivo). Instale para reativar a camada ASK."}\n' "'$1'"
  : > "$m" 2>/dev/null || true
}
...
main() {
  ...
  # Sem `grep -E` funcional toda deteccao daria "pass" — avisa em vez de fingir que verificou.
