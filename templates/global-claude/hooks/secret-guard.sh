...
#
# Irmao do destructive-guard.sh; usado quando NAO ha `pwsh`. Em cada Bash:
...
# camada de bloqueio real. Fail-safe ASSIMETRICO (igual ao .ps1). Sem `jq`, sem a lib ou sem
# `grep -E` funcional -> passthrough COM aviso por systemMessage, 1x/dia (_sdd_ask_degraded).
#
# Deteccao vem da lib UNICA lib/secret-patterns.sh (sourced). Paridade .ps1 x .sh — inclusive sob
# userland BSD (macOS) — verificada por tools/tests/secret-guard-sh.Tests.ps1.
#
# PORTABILIDADE (macOS): so' ERE (`grep -E`). O grep BSD nao tem `-P`; com o erro engolido por
# 2>/dev/null, este guard ficava inerte no macOS sem pwsh (medido no runner macos-15, 2026-09-10).
...
is_git_subcommand() {
  ...
  while IFS= read -r seg; do
    printf '%s' "$seg" | grep -qE "(^|[[:space:]])git[[:space:]]+([^[:space:]]+[[:space:]]+)*${sub}([[:space:]]|$)" && return 0
  done <<< "$norm"
  return 1
}
...
is_secret_read() {
  ...
  norm="$(printf '%s' "$cmd" | tr -d '\r' | sed -E 's/&&|\|\||;|\|/\n/g')"
  while IFS= read -r seg; do
    printf '%s' "$seg" | grep -qiE "(^|[[:space:]])${readers}[[:space:]]" || continue
    for tok in $seg; do
      t="${tok#[\"\']}"; t="${t%[\"\']}"
      [ -n "$t" ] || continue
...
# Aviso unico (1x/dia, NUNCA bloqueia) quando a camada ASK degrada por dep ausente.
# Sem isso, a falta de jq/lib desliga o guard em SILENCIO (fail-OPEN) — Y4 do e2e.
# Sai como systemMessage no STDOUT: stderr de hook com exit 0 vai so para o debug log (sonda com
# claude -p, 2026-09-12), e o dispatcher repassa o systemMessage de saida sem decisao. JSON fixo por
# printf -- o jq pode ser justamente o que falta; $1 e' sempre um rotulo literal deste arquivo.
_sdd_ask_degraded() {
  local m="${TMPDIR:-/tmp}/.sdd-ask-degraded.secret.$(date +%Y%m%d 2>/dev/null)"
  [ -e "$m" ] && return 0
  printf '{"systemMessage":"[sdd] aviso: %s ausente - guards interativos (.sh) INATIVOS (passthrough: segredo). Instale para reativar a camada ASK."}\n' "'$1'"
  : > "$m" 2>/dev/null || true
}
...
main() {
  ...
  # Sem `grep -E` funcional toda deteccao daria "nao" — avisa em vez de fingir que verificou.
  printf 'x\n' | grep -qE 'x' 2>/dev/null || { _sdd_ask_degraded 'grep -E'; exit 0; }
  ...
  # Daqui e' relevante: erro = ask (fail-safe assimetrico) — inclusive erro do motor de regex.
  ...
  if [ "$is_commit" -eq 1 ]; then
    if diff="$(git diff --cached --unified=0 2>/dev/null)"; then
      ...
      if ! names="$(find_secret_match "$added")"; then
