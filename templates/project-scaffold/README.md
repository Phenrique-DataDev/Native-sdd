...
.claude/
├── settings.json            registra os hooks curation-nudge + peer-heartbeat + complementary-repo-guard
...
| **Hooks** | `curation-nudge` (staleness da curadoria) + `peer-heartbeat` (presença p/ `/peers`) + `complementary-repo-guard` (ask no `PreToolUse` se mirar um repo complementar) — read-only/ask-only e fail-safe |
