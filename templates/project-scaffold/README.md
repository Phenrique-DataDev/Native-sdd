...
.claude/
├── settings.json            registra os hooks curation-nudge + peer-heartbeat + agent-graph-context
...
| **Hooks** | `curation-nudge` (staleness da curadoria) + `peer-heartbeat` (presença p/ `/peers`) + `agent-graph-context` (role+connects_to no `SubagentStart`) — read-only e fail-safe |
