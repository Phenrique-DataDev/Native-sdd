...
.claude/
├── settings.json            registra os hooks curation-nudge + peer-heartbeat + agent-graph-context + complementary-repo-guard
├── rules/                   15 regras SEMPRE aplicadas (contexto ativo — imposto de token em toda sessão)
│   ├── workflow-sdd · cli-first · docs-first · tooling                    fluxo SDD + otimizações
│   ├── agent-routing · orchestration                                      roteamento (núcleo) + orquestração
│   ├── doubt-driven · reflection · simulation · lessons · iterative-loop  posturas opt-in (/doubt /reflect /simulate /learn /iterate)
│   ├── complementary-repos                                                repos complementares de referência (/complementary-repos)
│   ├── documentation · kb-taxonomy                                        docs (fora da KB) + KB em 4 camadas
│   └── project-context.md   stack/convenções — preenchido via /setup
├── postures/                NÃO carregadas automaticamente — 0 token até um command lê-las
│   ├── max-mode.md          modo de operação máxima — lido pelo /max (que ABORTA se ele sumir)
│   └── agent-routing-advanced.md   grafo unificado · contrato de hub · gatilho ultracode (/max, /orchestrate)
...
│   ├── agent-graph-context     entrega role+connects_to ao subagente que inicia (SubagentStart)
...
| **Hooks** | `curation-nudge` (staleness da curadoria) + `peer-heartbeat` (presença p/ `/peers`) + `agent-graph-context` (role+connects_to no `SubagentStart`) + `complementary-repo-guard` (ask no `PreToolUse` se mirar um repo complementar) — read-only/ask-only e fail-safe |
