...
3. **Se existir `BUILD_REPORT` da feature em `.claude/sdd/reports/`, leia a seção
   `## Não exercitado` (ou `## Not exercised`, se o projeto escreve em inglês — o `sdd-lint`
   aceita as duas) ANTES de escolher onde olhar — e comece por ali.** É o mapa de cobertura
