...
## Execução
1. Quebre o DESIGN em tarefas pequenas e verificáveis (use a lista de tarefas). **A forma importa:**
   cada tarefa é um **tracer bullet** — caminho estreito e **completo** pelas camadas que a feature
   toca, verificável sozinha —, não uma fatia horizontal (*"primeiro todos os schemas"*), que só é
   verificável quando a última camada fecha. Prefactor que torna a mudança fácil é tarefa própria, e
   vem antes. **Exceção — *wide refactor*:** se a mudança é mecânica e o blast-radius quebra os
   call-sites de uma vez, nenhuma fatia vertical fecha verde; use **expand → migrate em lotes →
   contract** e leia a §*A forma da fatia* de `.claude/postures/orchestration.md` antes de sequenciar.
