...
**fase SDD em andamento**, **`inbox/` pendente**, **memória** (coerência da KB + auto-memória do
Claude), **staleness da curadoria** e **▶ próximo passo recomendado**. Pull on-demand — diferente do
`curation-nudge` (reativo, só staleness).
...
if ($toolsRoot) {
    ...
    Format-StatusReport (Get-StatusReport -Root .)
}
```
...
**inbox** (`Get-InboxItems`), **memória** (`Get-MemoryStatus` — KB via `Get-KbInventory` + ponteiros do
