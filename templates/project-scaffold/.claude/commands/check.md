...
que a curadoria gera/instala, e devolve um **veredito** de conformidade. Quatro seções: **kb** (frontmatter
...
> lints `kb-lint`/`agent-lint`/`config-lint`/`sdd-lint` (não reimplementa parsing). Seção sem alvo (ex.: KB vazia)
...
`Get-ProjectCheckReport` monta as 4 seções (cada uma **fail-safe**): **kb** (`Get-KbInventory` +
`Invoke-KbLint` sobre `.claude/kb`), **agent** (`Invoke-AgentLint` sobre `.claude/agents`), **config**
(`Invoke-ConfigLint` sobre `.claude/settings*.json`) e **sdd** (`Invoke-SddLint` sobre
`.claude/sdd/{reports,archive}`). `Get-CheckVerdict` (pura) agrega as severidades e
