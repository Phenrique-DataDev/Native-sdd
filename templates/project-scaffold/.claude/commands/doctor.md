...
Verifica se os **3 guards de segurança** (`main-push` / `secret` / `destructive`) **realmente
...
> **Read-only.** Não escreve em `~/.claude` nem toca produção. Só cria/derruba um repo git **efêmero**
> em `$TEMP` para o dry-run do push-guard. Roda no `pwsh` (que funciona quando você o invoca) e
> **prova o comportamento** dos guards instalados.
...
`rm -rf /`; `main-push` → `git push` num repo git temporário na branch default) e confere a
`permissionDecision`. Cada achado é `ok` · `fail` (gate reprova) · `skip` (não-aplicável, ex.: `git`
ausente para o push-guard).
...
- **settings:** `~/.claude/settings.json` tem `hooks.PreToolUse` com os 3 guards, e os paths
...
- **`skip` não é `fail`:** ausência de `git` pula só o dry-run do push-guard (os outros dois seguem).
