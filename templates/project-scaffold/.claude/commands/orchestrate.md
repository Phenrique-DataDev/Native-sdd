...
**Este passo vem primeiro, e a ordem é o conserto.** Até 2026-09-19 as duas posturas eram lidas
**antes** daqui: ~19.400 tokens pagos para depois ser informado de que não se deve orquestrar. Medido
em sessão real: `/orchestrate` custava **+23%** sobre o mesmo objetivo sem comando nenhum, e em 11
sessões o fan-out não emergiu **nenhuma vez** — o regime `solo` é o desfecho comum, por construção do
próprio predicado. Pagar a postura antes de medir contradiz a razão pela qual ela saiu do always-on.
