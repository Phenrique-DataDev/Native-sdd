...
| `.claude/agents/graph.json` + `graph.cypher` | o **grafo unificado** (**H9**): agentes (`role`/`connects_to`) **+ KB** (`:KbEntry`, `:IN_DOMAIN`) **+ skills** (`:Skill`/`:PRESUPPOSES`) **+ domínios** (`:Domain`) **+ `:USES_SKILL`** **+ o `:Hub`** orquestrador-mestre (`:ORCHESTRATES` a todos — o **ápice**). JSON p/ consulta, Cypher p/ neo4j (`[[H4]]`) |
...
regenerável). O `graph.cypher` segue pronto p/ o neo4j **se** o volume um dia justificar (`[[H4]]`).
