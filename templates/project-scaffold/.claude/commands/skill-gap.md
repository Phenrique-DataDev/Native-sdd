...
Para cada skill recém-criada, **autore** (LLM) as seções `TODO` (Quando usar · Passos · Gotchas · Se
crescer) a partir da capacidade e do contexto do projeto. Mantenha `status: scaffolded` até revisão
humana — o conteúdo gerado é **proposta**, não verdade final.
...
| Skill é **pasta**, não só markdown | se precisa de código reusável, ponha em `scripts/` (função pronta que o Claude compõe) em vez de instruir a reescrever toda vez |
...
| Comece pequeno | poucas linhas + 1 gotcha real; cresce por iteração conforme surgem casos — não tente esgotar de uma vez |
