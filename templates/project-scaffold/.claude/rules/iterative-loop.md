...
O Claude Code reconhece 4 tipos de loop: **turn-based** (manual), **goal-based** (`/goal`, para na
condição de sucesso ou no teto de tentativas), **time-based** (`/loop`/`/schedule`, por intervalo/cron)
e **proactive** (combina os anteriores + `Workflow` + auto mode). O `/iterate` **é** um goal-based loop
— só **restrito à classe segura** abaixo e **sandboxed** (`isolation:'worktree'`); não substitui o
`/goal` de propósito geral. Fora da classe elegível, use o primitivo direto:
...
A fronteira de segurança (mutador/outward fica no loop principal, sob humano) vale igual para os
primitivos nativos — `/iterate` só a formaliza **dentro** da classe segura.
