...
**Entrega automática do grafo de pares (hook, não só instrução).** A leitura acima ("antes de rotear,
consulte o grafo") depende do **líder** lembrar de abrir o `graph.json` — na prática isso é confiança de
prompt, não um mecanismo. O hook **`agent-graph-context`** (`.claude/hooks/agent-graph-context.ps1`,
`SubagentStart`) fecha essa lacuna para a fatia `role`/`connects_to`: a cada subagente que **inicia**, o
hook injeta via `additionalContext` o `role` dele e os alvos do próprio `connects_to` — determinístico,
sem depender do líder. Escopo **deliberadamente estreito** (só o grafo de pares, reusando
`Get-AgentGraph`): dispara em todo subagente, então o custo por disparo fica baixo — o grafo
**unificado** (KB/skills/domínio) continua sendo consulta sob demanda do líder, não algo injetado aqui.

**Sem backstop automático de staleness.** O `resync-lint` (que checava `graph.json`/`AGENT_MAP.md`/
`_index.yaml` contra o estado real) saiu do `check.ps1`/CI na postura low-friction (2026-06-20, ver
`CHANGELOG.md`). Hoje **nada** verifica automaticamente se o grafo está sincronizado com o frontmatter
dos agentes — rodar `/sync-context` após curar é responsabilidade de quem cura, não uma rede de
segurança do CI. `agent-lint` continua checando **integridade referencial** (`connects_to` aponta a
agente existente), mas isso é diferente de "o grafo reflete o estado atual".
...
| "Quem mais está mexendo neste projeto?" (sessões concorrentes) | `/peers` | quadro **file-based** de coordenação entre sessões: lista peers ativos (branch/summary/heartbeat) + caixa de recados; escrito pelo hook `peer-heartbeat`, lido on-demand — `tools/peers.ps1`. Sem daemon/rede. Distinto do `/status` (estado do próprio projeto) e do `curation-nudge` (staleness) |
...
| Iterar uma meta verificável até o verde (loop bounded) | `/iterate` | laço **loop-until-green** **só** na classe segura (determinística/reversível/machine-verifiable): `Test-IterateEligible` (rejeita schema/IaC/dados/outward) + `Test-IterateVerifiable` (degrada sem sinal) → **Workflow nativo** (`isolation:'worktree'` + `budget`) com `Get-VerifierResult`→`Test-TaskGate` reusado + stuck-detection. Protocolo na regra `iterative-loop.md`; muta só no worktree, merge é humano. Distinto do `/orchestrate` (DAG, `MaxAttempts=2`) |
