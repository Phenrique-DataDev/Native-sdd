...
No **modo MAX** (`/max`, ver [`max-mode.md`](max-mode.md)) o líder opera como **hub** desse grafo — o
**orquestrador-mestre** que delega a experts via `/orchestrate` e consulta `role`/`connects_to` para
encadear. O MAX **prepara** o hub usando os metadados que já existem (0-dangling); o grafo consultável
(neo4j, H4) fica para depois.
