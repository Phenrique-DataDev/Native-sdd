...
> **Células curtas de propósito:** o harness já injeta o `description` de todo agente (2.105 tok, always-on) — aqui vai só a **fronteira com o vizinho confundível**. Orçamento ~12 palavras/linha, travado pelo `agent-table-lint`.
...
mecanismo nenhum (um hook `agent-graph-context` fazia isso até 2026-09-01 e foi **removido** — o A/B
mostrou que a decisão não mudava, porque esta rule já é always-on e basta).
