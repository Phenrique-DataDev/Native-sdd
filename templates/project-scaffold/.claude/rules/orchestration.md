...
# `completo` só sai com -ReductionAttempted $true — ver abaixo.
```
...
| **`completo`** | **Leia** `.claude/postures/orchestration.md` e rode o ciclo inteiro. | Material não cabe **e** a redução foi tentada e falhou |

> Os sufixos **compõem** (`minimo+worktree+checkpoint`): fazer um vencer o outro descarta o
> mecanismo do perdedor.

**A fronteira de máquina de estado é UMA só, e ela cobra um fato.** O material **não cabe** no
contexto de um agente **e** a redução falhou — `-ReductionAttempted $true` quer dizer que você
escreveu a ferramenta que resolveria o material e ela não deu conta. `-FitsSingleContext $false`
sozinho é **auto-relato** (quem informa é o líder que já quer decompor: o defeito do gatilho anterior
com nome novo), e a função devolve o modo barato com a instrução dentro da `Reason` — a regra é
cobrada, não lembrada. **Volume não é a pergunta; irredutibilidade é.** **Contar disciplinas não é
fronteira**, e **isolamento também não**: ele pede worktree, não máquina de estado. Todos medidos;
derivação na postura.
...
|-------|-------------|
| **`/orchestrate <objetivo>`** | Siga-o. O **Passo 0 dele é rodar este predicado** e dizer o `Mode` ao usuário; a postura só é carregada no Passo 1, e só se o regime não for `solo` — medir custa uma chamada de função pura, a postura custa ~19.400 tokens. |
