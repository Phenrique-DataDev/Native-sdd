...
Grep/glob são ótimos quando você sabe o termo exato. Prosa acumulada ao longo de muitos ciclos
SDD (KB, `docs/`, `archive/`) cresce em volume e vocabulário — uma pergunta como *"onde
decidimos usar X em vez de Y?"* pode não bater em nenhuma palavra-chave previsível. O MCP
`semantic-kb` (Ollama local + `sqlite-vec`, **opt-in**, nunca um processo em segundo plano —
índice é artefato gerado, regenerado por reindexação incremental) existe para esse caso
específico: busca por significado, não por string exata.
