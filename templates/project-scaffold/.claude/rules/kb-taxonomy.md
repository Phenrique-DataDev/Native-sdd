...
- O **mesmo princípio advisory** vale para o **contexto always-on** das `.claude/rules/` (não a KB):
  `tools/rules-budget.ps1` (G8) mede o footprint sempre-ativo e sinaliza drift sem bloquear. Distinto
  desta seção (que orça **entradas de KB**, carregadas sob demanda).
