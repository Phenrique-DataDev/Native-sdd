...
4. **Backup antes de aplicar** — só após aprovação; faça backup das entradas afetadas **fora de
   `.claude/kb/`** (`.claude/.cache/kb-backup/<data>/`, gitignored). Backup **dentro** da KB (ex.: ao lado
   do plano, em `_reflections/`) é ignorado pelo lint e **duplica o `id`** no grafo (`kb-lint`:
   `misplaced-entry`, warn).
