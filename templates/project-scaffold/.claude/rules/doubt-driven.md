...
raciocínio anexado, convida o viés de confirmação mesmo num subagente. O `doubt-driven` inverte a
pergunta: monta o caso **sem** a conclusão e instrui um revisor **fresh-context** a *"achar o que está
errado"*. O valor é a postura adversarial in-flight + a omissão da conclusão. Sem engine: o
fresh-context é a ferramenta `Agent` nativa (igual `orchestration.md`).
