...
> reaplicável. O `[[ship]]` arquiva "Lições aprendidas" em `SHIPPED_*.md` — acervo que ninguém relê;
> o `[[reflection]]`/`/reflect` **consolida** o que já está na KB (não capta novo). O `/learn` fecha o
...
`[[ship]]`. O gatilho conta as `[candidata]` **ainda não promovidas**.
...
2. **Por domínio** — agrupe as candidatas pendentes por domínio; fan-out via `Agent` (1 subagente por
   domínio, DNA das ondas do `[[train-kb]]`), contexto limitado.
