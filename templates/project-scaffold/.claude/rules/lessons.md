...
> reaplicável. O [`/ship`](../commands/ship.md) arquiva "Lições aprendidas" em `SHIPPED_*.md` — acervo
> que ninguém relê. O `/learn` fecha esse loop: relê o acervo, deixa o LLM detectar a lição
> **recorrente** (repete entre features) e a **promove** a uma entrada de KB `operations`. Distinto do
> [`reflection.md`](reflection.md) (`/reflect`, consolida o que já está na KB — não capta novo).
> **Nunca-destrutivo** (plano → aprova → aplica), com proveniência (`promoted_from`); sem engine —
> gatilho e verificação determinísticos (`tools/learn.ps1`, reusa `kb-lint`/B7).
...
5. **Backup antes de gravar** — só após aprovação; faça backup de qualquer entrada afetada **fora de
   `.claude/kb/`** (`.claude/.cache/kb-backup/<data>/`, gitignored). Backup **dentro** da KB (ex.: ao lado
   do plano, em `_lessons/`) é ignorado pelo lint e **duplica o `id`** no grafo — o `kb-lint` avisa
   (`misplaced-entry`, warn), mas o lugar certo é fora.
