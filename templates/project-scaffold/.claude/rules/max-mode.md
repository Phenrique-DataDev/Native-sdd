...
| Trabalho de **disciplina** com suplemento no repertório (design · data · security · dev · ai · meta) | **suplemento do tema** (`/supplements`, skills opt-in, `-ExtraPlugins -Themes`) | local | **auto-fire só na disciplina do tema** |
...
> `design` (impeccable/ui-ux-pro-max), `reporting` (visual-explainer), `data`, `security`, `dev`, `ai`, `meta`.
