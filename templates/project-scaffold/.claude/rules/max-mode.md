...
|-------------------|-----|---------|
| Poucas tasks **sequenciais com gate** / encadeamento dependente | **`/orchestrate`** (`[[orchestration]]`, `Agent` síncrono) | gate determinístico + re-invocação com feedback; o líder coleta cada `result` |
...
task for desse tipo. Reusa `Build-KbIndex` (`[[kb-taxonomy]]`/G4) — **sem nova varredura**. O bootstrap já
...
um Workflow — **não declara nem inventa um número de orçamento**. Coerente com o `[[kb-taxonomy]]` (orçamento
