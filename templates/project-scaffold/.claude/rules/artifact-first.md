...
| **Rotear** | Confirme pela tabela acima se `decision-preview` é mesmo a ferramenta certa (ou se `visual-explainer`/`artifact-design` já bastam). O gatilho explícito é [`/decision-preview`](../commands/decision-preview.md) — não espere lembrar da skill sozinho. |
