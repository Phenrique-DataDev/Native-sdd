...
- **Working-tree:** `rg` dirigido a padrões de alto valor — `AKIA`/`ASIA`, `-----BEGIN.*PRIVATE KEY-----`, `xox[baprs]-`, `ghp_`/`github_pat_`, `sk-`, `eyJ` (JWT), `password\s*=`, `api[_-]?key`, connection string com senha embutida (`postgres://` / `mysql://` seguido de `usuário:senha` antes do `@`). Combine regex com **entropia** para não afogar em falso-positivo.
