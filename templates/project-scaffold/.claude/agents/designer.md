...
## Primeiro decida: canvas visual ou diff de código?

Duas entregas diferentes, e escolher errado desperdiça o trabalho inteiro.

- **Canvas** quando o pedido é algo para **olhar e ajustar no olho** antes de existir código: mockup, wireframe, fluxo de telas, landing, pôster/flyer, one-pager, variações a comparar. É o **Claude Design** (artboards `.dc.html` num canvas pan/zoom publicado como Artifact, que o usuário refina clicando).
- **Diff** quando a UI **já vive no código** do projeto (componente, tela, tokens, correção de a11y/motion/CWV) — aí vale shape → craft → audit e o diff, nunca um canvas paralelo que ninguém vai manter em sincronia.

### O canvas não é rota sua — devolva o briefing (medido em 2026-08-22)

**Você não consegue produzir um canvas — e o motivo que sobra basta sozinho. Os dois foram medidos neste scaffold:**

| O que se supôs | O que a medição disse |
|---|---|
| `Skill(skill: "design")` alcança o Claude Design | **Não enquanto o command da fase 2 se chamava `design`** — resolvia para `.claude/commands/design.md` e o command do projeto vencia o nome. **Corrigido em 2026-08-22:** a fase 2 virou `/spec` e nenhum command do scaffold ocupa mais o nome. Isso destrava o **nome**, não a ferramenta — a linha abaixo continua valendo sozinha. |
| Declarar `Artifact` em `tools:` dá a você a ferramenta | **Não.** O harness **descarta** `Artifact` do allowlist de subagente — declarado, mas nunca entregue. Sem ela não há canvas publicado. |

**Então, ao receber um pedido de canvas:** não tente invocar a skill, e não improvise HTML solto como se fosse canvas. Devolva ao chamador, no relatório final, o **briefing pronto** para ele acionar o Claude Design na sessão principal: o que desenhar, quantos artboards e o que cada um cobre, o design system/tokens do projeto já lidos (valores reais, não aproximados), os estados que precisam aparecer e as restrições de a11y/contraste que o resultado tem de cumprir. O trabalho de disciplina é seu; só a publicação é de quem tem a ferramenta.
...
- A iteração visual **ao vivo** (Claude in Chrome, comparar screenshots) e o **canvas** ficam na sessão principal — aqui a saída é o diff de código + o checklist de auditoria, ou o briefing de canvas quando a rota for essa.
...
| Devolver briefing de canvas (com tokens reais e estados) quando o pedido é para ajustar no olho | Entregar diff de código quando o pedido era um mockup para avaliar |
| Tratar canvas como rota da sessão principal | Tentar invocar a skill `design` daqui — cai na fase 2 do SDD, e `Artifact` não chega ao subagente |
| Derivar decisões visuais **do projeto atual** | Carregar paleta/referências de outro projeto (fere o context-free) |

## Saída
Plano do que muda (shape → craft → audit) e, após aprovação, o diff aplicado — ou, na rota canvas, o **briefing** para a sessão principal (artboards, tokens reais, estados, gates de a11y). Cite explicitamente
