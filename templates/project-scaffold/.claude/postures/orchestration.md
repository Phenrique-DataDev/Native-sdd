...
| Modelo | o modelo sugerido — **alias de família** (`opus` para trabalho denso), nunca a versão; ver `rules/agent-routing.md` §*Política de modelo* |

### A forma da fatia — o que é uma task bem cortada

Antes de *como atacar*, **onde cortar**. Uma task bem cortada é um **tracer bullet**: um caminho
**estreito e completo** por todas as camadas que a feature toca (schema → serviço → interface →
teste), não uma fatia horizontal de uma camada só.

| Propriedade | Teste rápido |
|---|---|
| **Vertical** | atravessa todas as camadas envolvidas, ainda que por um caso só |
| **Verificável sozinha** | dá para demonstrar/rodar a task isolada, sem esperar as irmãs |
| **Cabe num contexto fresco** | um agente que só recebe o pacote de contexto consegue fechá-la |
| **Prefactor primeiro** | o preparo que torna a mudança fácil é task **própria**, e vem antes — *make the change easy, then make the easy change* |

Fatia horizontal (*"primeiro todos os schemas, depois todos os endpoints"*) parece organizada e
esconde o risco no fim: nada é verificável até a última camada fechar, e o gate não tem o que cobrar
nas anteriores.

#### A exceção: *wide refactor* não vira tracer bullet

**Predicado observável:** a mudança é **mecânica e única** (renomear uma coluna, retipar um símbolo
compartilhado) e o **blast-radius** atravessa a codebase — uma edição quebra milhares de call-sites de
uma vez. Aí **nenhuma** fatia vertical fecha verde, e insistir na regra acima produz uma sequência de
tasks que reprovam todas.

Sequência **expand → migrate → contract**, cada etapa uma task com `deps` explícito:

| Etapa | O que faz | `deps` |
|---|---|---|
| **expand** | acrescenta a forma nova **ao lado** da velha — nada quebra | — |
| **migrate** | converte os call-sites em **lotes dimensionados pelo blast-radius** (por pacote, por diretório); cada lote é uma task, e o CI fica verde entre lotes porque a forma velha ainda existe | `[expand]` |
| **contract** | remove a forma velha quando não sobra chamador | `[migrate-1, migrate-2, …]` |

**Quando nem os lotes fecham verde sozinhos:** mantenha a sequência, mas deixe-os compartilhar um
**branch de integração** que todos bloqueiam, e ponha uma task final de *integrar e verificar* — o
verde é prometido **ali**, e isso fica escrito no `dod`, não subentendido.

> **Procedência:** método adaptado da skill `to-tickets` de [`mattpocock/skills`](https://github.com/mattpocock/skills)
> (MIT), avaliada em 2026-08-28. A **skill** não foi adotada: a decomposição, as arestas de bloqueio e a
> *frontier* já são o Passo 2 + `ReadyTasks` + STATE deste protocolo, e um segundo motor ao lado
> reintroduziria o defeito que o `ORQUESTRA_PRESCREVE_TOPOLOGIA` fechou. O que faltava era só a **forma
> da fatia** — e ela cabe aqui, ao lado do método, sem custar always-on.
...
- **Padrão — subagente na mesma sessão** (`Agent`): síncrono/paralelo/resume; o líder coleta o
  `result` e valida.
- **Handoff externo** — emitir a instrução *"plano aprovado, rode em novo chat no modelo mais capaz
  disponível"* quando:
...
tasks:
  - id: decompor
    ...
    model: opus              # alias de FAMILIA, nunca a versao (rules/agent-routing.md)
    deps: []
