...
material é **redutível**. A pergunta que discriminaria é *"é irredutível?"* — e ela **não virou
parâmetro novo**, de propósito: trocar um sinal que não sei medir por outro que também não sei medir
só muda o nome do problema. O que virou produto é o **corolário utilizável**: antes de informar
`-FitsSingleContext $false`, pergunte se um script resolveria; se resolver, o material cabe numa
ferramenta. Três tentativas de fabricar irredutibilidade, três fracassos — a fronteira de volume
**nunca foi exercida com sucesso**, e o gatilho de reabertura é alguém achar uma carga que resista a
um parser.
