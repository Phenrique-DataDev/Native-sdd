...
- [`artifact-first.md`](.claude/rules/artifact-first.md) — **comparar variantes antes de construir**: oferece um Artifact (skill `decision-preview`) quando a decisão tem ≥2 opções viáveis e é cara de refazer, evitando redundância com `artifact-design`/`visual-explainer`
- [`semantic-search.md`](.claude/rules/semantic-search.md) — **busca semântica opt-in** (MCP `semantic-kb`, Ollama+`sqlite-vec`) sobre KB/`docs/`/`archive/` quando a pergunta é linguagem natural sobre prosa — complementa `Grep`/`Glob`, nunca substitui; degrada em silêncio se o MCP não estiver instalado
...
- [`complementary-repos.md`](.claude/rules/complementary-repos.md) — **repositórios complementares de referência**: consulta sob demanda (read-only), nunca vendoriza; boundary reforçado pelo hook `complementary-repo-guard` (`/complementary-repos`)
