...
| [`agent-routing-advanced.md`](.claude/postures/agent-routing-advanced.md) — grafo unificado, hub, `ultracode` | `/orchestrate` (Passo 1, se o regime pedir) e `/max` (Passo 3, ao encadear) |
| [`orchestration.md`](.claude/postures/orchestration.md) — protocolo do líder: ciclo, gate, checkpoint, paralelismo, STATE | `/orchestrate`, no Passo 1 (só se o regime ≠ `solo`) — **aborta** se sumir |
...
> **Rules condicionais.** Quatro rules não carregam a disciplina inteira: elas guardam um **predicado
> observável** (o registro existe? a pergunta é paráfrase sobre prosa? a KB tem entradas? vou
> orquestrar?) e mandam ler a postura quando ele é verdadeiro. Uma rule é porta legítima
