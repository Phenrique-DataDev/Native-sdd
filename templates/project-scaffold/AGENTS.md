...
- [`semantic-search.md`](.claude/rules/semantic-search.md) — **predicado**: o MCP `semantic-kb` existe? disciplina em `postures/`
...
| [`semantic-search.md`](.claude/postures/semantic-search.md) — quando/como buscar por significado | a rule (se o MCP estiver conectado) |

> **Rules condicionais (2026-08-10).** Quatro rules deixaram de carregar a disciplina inteira em toda
> sessão: elas guardam um **predicado observável** (o registro existe? o MCP está conectado? a KB tem
> entradas? vou orquestrar?) e mandam ler a postura quando ele é verdadeiro. Always-on medido:
