...
posix_apply_main() {
  ...
  local THEMES="" MANAGED_POLICY="" WITH_SKB=0 SKB_MODEL="" SKB_HOST=""
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --with-semantic-kb)           WITH_SKB=1 ;;
      --semantic-kb-model)          shift; SKB_MODEL="${1:-}" ;;
      --semantic-kb-model=*)        SKB_MODEL="${1#*=}" ;;
      --semantic-kb-ollama-host)    shift; SKB_HOST="${1:-}" ;;
      --semantic-kb-ollama-host=*)  SKB_HOST="${1#*=}" ;;
  ...
  [ "$WITH_SKB" = 1 ]         && PS_ARGS+=(-WithSemanticKb)
  [ -n "$SKB_MODEL" ]         && PS_ARGS+=(-SemanticKbModel "$SKB_MODEL")
  [ -n "$SKB_HOST" ]          && PS_ARGS+=(-SemanticKbOllamaHost "$SKB_HOST")
