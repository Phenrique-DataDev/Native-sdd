...
function Invoke-PluginsSetup {
    ...
    # Pelo onboarding, plugin nasce DESABILITADO (decisão de 2026-09-11): o repertório inteiro custava
    # ~10,1k tok always-on + 1 MCP + 6 hooks em toda sessão. O usuário liga pelo /plugin o que usar.
    Invoke-SupplementsSetup -Summary $Summary -Themes $Themes -Disabled -Check:$Check -DryRun:$DryRun
}
