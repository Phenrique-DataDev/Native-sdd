...
# --- Estado do Ollama (fonte única do MCP semantic-kb) -------------------
# POR QUE ISTO EXISTE (achado em uso real, 2026-07-16): os instaladores de MCP local decidiam presença do
# Ollama com `Test-CommandExists 'ollama'` — que responde "o BINÁRIO está no PATH?", não "o SERVIÇO
# responde?". Com o Ollama instalado e o serviço PARADO (o estado normal de quem reiniciou e não
# abriu o app), a cascata inteira mentia: o plano dizia 'ambiente pronto'; `ollama list` falhava por
# conexão recusada e devolvia $false — INDISTINGUÍVEL de "modelo ausente"; o plano virava 'pull';
# o pull tentava baixar um modelo QUE JÁ ESTAVA NO DISCO e falhava; e o WARN mandava "baixe
# manualmente depois" — orientação que falharia igual, porque o problema nunca foi o download.
# Medido: nomic-embed-text já estava na máquina havia 11 dias.
# A lição é a mesma que já mordeu aqui: um predicado que responde a pergunta ERRADA não fica verde
# nem vermelho — ele fica plausível. 'stopped' é o estado que faltava para o diagnóstico não mentir.
function Test-OllamaServing {
    # EFEITO (rede): o serviço do Ollama responde no host? A raiz devolve 200 ('Ollama is running').
    # Timeout curto de propósito: isto roda no caminho do instalador, e um Ollama remoto inacessível
    # não pode travar o onboarding. Qualquer falha (conexão recusada, DNS, timeout) = $false.
    param(
        [string]$OllamaHost = 'http://localhost:11434',
        [int]$TimeoutSec = 3
    )
    if ([string]::IsNullOrWhiteSpace($OllamaHost)) { return $false }
    try {
        $null = Invoke-WebRequest -Uri $OllamaHost -TimeoutSec $TimeoutSec -UseBasicParsing -ErrorAction Stop
        return $true
    }
    catch { return $false }
}
...
function Get-OllamaState {
    # PURA. Cruza as DUAS perguntas que estavam fundidas numa só e devolve o estado real:
    #   'missing' -> não há CLI: nada a fazer sem instalar (winget install Ollama.Ollama)
    #   'stopped' -> CLI existe, serviço não responde: `ollama serve` resolve — NÃO é falta de modelo
    #   'ok'      -> CLI existe e o serviço responde: só aqui `ollama list`/`ollama pull` são confiáveis
    # Sem CLI o estado é 'missing' mesmo com um host remoto no ar: `ollama pull` precisa do binário
    # local para rodar, então "serviço remoto respondendo" não habilita nenhum passo daqui.
    param(
        [Parameter(Mandatory)][bool]$CliPresent,
        [Parameter(Mandatory)][bool]$Serving
    )
    if (-not $CliPresent) { return 'missing' }
    if (-not $Serving)    { return 'stopped' }
    return 'ok'
}
...
function Install-BaselineItem {
    ...
    if (([System.IO.Path]::GetExtension($Item.Dst)) -ieq '.json') {
        Install-JsonBaselineItem -Item $Item -Summary $Summary -HomePath $HomePath -NativeHooks $NativeHooks -JsonBaselineDir $JsonBaselineDir -Check:$Check -DryRun:$DryRun
        return
    }
...
function Test-ScaffoldRelInProfile {
    ...
    # Rules: só o núcleo. É o grosso da economia (~45k chars ≈ 20k tokens always-on).
    ...
    if ($r -match '^\.claude/hooks/') { return $false }
