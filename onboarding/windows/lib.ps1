...
function Install-JsonBaselineItem {
    ...
    try {
        ...
        # Reescreve hooks p/ runtime nativo (pwsh) — só no Windows (J4), onde `sh` não é garantido.
        # Em Linux/macOS mantemos a dispatch-line portável do template (sh -c '… pwsh || bash …').
        if ($NativeHooks) { $merged = ConvertTo-NativeHooks $merged }
