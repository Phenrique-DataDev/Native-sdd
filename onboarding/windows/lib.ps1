...
# --- Estado do Ollama (fonte única dos dois MCP: local-ai e semantic-kb) --
# POR QUE ISTO EXISTE (achado em uso real, 2026-07-16): os dois instaladores decidiam presença do
...
function Expand-BaselinePlaceholder {
    ...
    # mesmo defeito (esse sim vivo) em Get-HerdrHostArch/Get-CliProxyHostArch.
    param(
        [string]$Text,
...
#    usuário, instalados pelo onboarding) e o projeto os herda — verificado em 2026-07-17. Os 4 hooks
#    do scaffold (curation-nudge, peer-heartbeat, agent-graph-context, complementary-repo-guard) são
...
function Read-ScaffoldManifest {
    ...
    return (ConvertFrom-ManifestText -Lines (Get-Content -LiteralPath $file -ErrorAction SilentlyContinue))
}
...
function Read-BaselineManifest {
    ...
    return (ConvertFrom-ManifestText -Lines $lines)
}
