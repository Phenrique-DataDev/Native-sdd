...
# permanecem intactos. Mesma disciplina de install-semantic-kb.ps1.
#
...
# Separado de propósito: é o ÚNICO ponto que executa um binário de terceiro, e a suíte o mocka.
...
function Invoke-HerdrSetup {
    ...
    try {
        ...
        if (Test-HerdrAssetIsArchive -AssetName ([string]$asset.AssetName)) {
            ...
            if (-not (Expand-HerdrArchive -ArchivePath $tmp -DestinationDir $destDir -BinaryName (Get-HerdrBinaryName))) {
                Write-Step WARN "herdr: o archive $($asset.AssetName) não produziu $(Get-HerdrBinaryName) — abortado, nada instalado"
                $Summary.Warn++
                return
            }
