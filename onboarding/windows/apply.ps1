...
if ($currentVersion) {
    ...
    if ($upToDate) {
        ...
        # ...mas "a versão bate" não é "os arquivos estão lá". O manifest sabe o que entregamos;
        # conferir a EXISTÊNCIA custa um Test-Path por linha e transforma um verde mentiroso num
        # aviso acionável. Só avisa — reparar sozinho exigiria respeitar as classes de preservação,
        # e isso é outra fatia. Ver Get-MissingBaselineFile para o caso medido que originou isto.
        $sumidos = @(Get-MissingBaselineFile -Path $DestRoot)
        if ($sumidos.Count -gt 0) {
            Write-Step WARN "$($sumidos.Count) arquivo(s) do baseline sumiram do disco — rode com -Force para repor:"
            foreach ($s in $sumidos) { Write-Step WARN "  [faltando] $s" }
            $summary.Warn++
        }
    }
} else {
...
if ($upToDate) {
    ...
    if (-not $map -or @($map).Count -eq 0) {
        ...
        foreach ($item in $map) {
            Install-BaselineItem -Item $item -Summary $summary -HomePath $userHome -NativeHooks $nativeHooks -JsonBaselineDir $jsonBaselineDir -Manifest $baselineManifest -PreserveCustom -OverwriteCustom:$OverwriteCustom -Check:$Check -DryRun:$DryRun
        }
        ...
        if (-not $Check -and -not $DryRun) {
            # O que foi PRESERVADO não entra no manifest com o hash do disco — senão a
            # customização vira "foi isto que entregamos" e some na rodada seguinte.
            $preservados = @()
            if ($summary.ContainsKey('Preserved')) { $preservados = @($summary.Preserved) }
            Write-BaselineManifest -Path $DestRoot -Content (Get-BaselineManifestContent -Map $map -Previous $baselineManifest -Skip $preservados) -Summary $summary
        }
    }
}
...
elseif ($os -eq 'Windows') {
    ...
    # macOS: o shim acima chama pwsh e o claude mora em ~/.local/bin, mas o brew não entra no PATH de
    # shell novo sem `brew shellenv`. Bloco próprio no ~/.zprofile (ver Get-BrewShellenvBlock).
    if ($os -eq 'macOS') {
        Install-BashShim -RcPath (Join-Path $userHome '.zprofile') -RepoRoot $RepoRoot -Block (Get-BrewShellenvBlock) `
            -MarkerStart $script:BrewMarkerStart -MarkerEnd $script:BrewMarkerEnd -Label 'Homebrew/PATH' `
            -Summary $summary -Check:$Check -DryRun:$DryRun
    }
}
...
# público herdrdev/herdr): baixa o asset da versão pinada (onboarding/herdr/herdr.psd1),
...
if ($WithHerdr) {
    ...
    # No Windows, sem `default_shell` todo pane do herdr abre no Windows PowerShell 5.1, onde o nsp não
    # existe (ver install-default-shell.ps1, bloco do herdr). Quem pediu o herdr pediu panes utilizáveis.
    if ($os -eq 'Windows') {
        . (Join-Path $PSScriptRoot 'install-default-shell.ps1')
        Invoke-HerdrDefaultShellSetup -Summary $summary -HerdrInstalled -Check:$Check -DryRun:$DryRun
    }
}

# --- A2h: shell padrão do Windows Terminal e do herdr (OPT-IN via -SetDefaultShell) ----
...
# O herdr é a MESMA porta por outro caminho: os panes dele não herdam o shell do terminal.
# Opt-in de propósito: mexe numa ferramenta do usuário. Falha = WARN, e faz backup antes.
if ($SetDefaultShell) {
    Write-Host ''
    Write-Step INFO 'Shell padrão do Windows Terminal e do herdr (opcional)'
    . (Join-Path $PSScriptRoot 'install-default-shell.ps1')
    Invoke-DefaultShellSetup -Summary $summary -Check:$Check -DryRun:$DryRun
    # Com -WithHerdr o bloco acima já tratou o herdr nesta execução.
    if ($os -eq 'Windows' -and -not $WithHerdr) {
        Invoke-HerdrDefaultShellSetup -Summary $summary -Check:$Check -DryRun:$DryRun
    }
}
