...
# --- A2e: plugins extra (OPT-IN via -ExtraPlugins, NÃO bloqueante) ---------
# Instala plugins de DOMÍNIO/superfície opcional (design, reporting) user-scoped ("global
# dormente"): disponíveis em todo projeto, auto-ativam só no trabalho da categoria. Opt-in de
# propósito p/ não acoplar o scaffold a um domínio (V2). Falha = WARN, nunca quebra A1/A2.
if ($ExtraPlugins) {
    Write-Host ''
    Write-Step INFO 'Plugins extra (opt-in, user scope)'
    . (Join-Path $PSScriptRoot 'install-plugins.ps1')
    Invoke-PluginsSetup -Summary $summary -Check:$Check -DryRun:$DryRun
}
