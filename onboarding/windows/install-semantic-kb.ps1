...
# WARN, nunca Failed — A1 (CLIs) e A2 (baseline ~/.claude) permanecem intactos. Sem checagem de
# VRAM: o modelo de embedding é leve (~274MB) e não precisa de aviso de hardware.
...
function Get-SemanticKbPlan {
    <#
    .SYNOPSIS
        Decide (puro) o plano de provisionamento do semantic-kb a partir do ambiente —
        sem campo de hardware/VRAM.
    .OUTPUTS
        [pscustomobject] com:
...
function Invoke-SemanticKbSetup {
    ...
    .NOTES
        ...
        modelo é leve (~274MB) mas só roda fora de -Check/-DryRun e quando o Ollama está
        presente.
    #>
    [CmdletBinding()]
    param(
