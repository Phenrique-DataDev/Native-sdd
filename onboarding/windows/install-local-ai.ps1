...
function Get-LocalAiPlan {
    ...
    param(
        ...
        # 'stopped' (CLI presente, serviço parado) é um estado de PRIMEIRA CLASSE — ver Get-OllamaState
        # em lib.ps1. Aqui o dano do estado colapsado era pior que no semantic-kb: o pull inútil baixaria
        # um modelo de GIGABYTES que já está no disco.
    ...
    # falha e "não achei o modelo" é indistinguível de "não consegui perguntar".
    $pull =
...
function Invoke-LocalAiSetup {
    ...
    # Só consulta o modelo quando a resposta é confiável (serviço no ar) — ver Get-LocalAiPlan.
    $modelPresent  = if ($ollamaState -eq 'ok') { Test-LocalAiModelPresent -Model $Model } else { $false }
    ...
    elseif ($plan.Ollama -eq 'stopped') {
        Write-Step WARN "local-ai: Ollama instalado mas não responde em $OllamaHost — inicie o serviço (``ollama serve``) e rode de novo; o modelo pode já estar baixado"
        $Summary.Warn++
    }
