        ...
        if ($backend -eq 'proxy') {
            ...
            foreach ($e in $catalog) {
                if ([string]::IsNullOrWhiteSpace($e['Provider'])) {
                    $errors.Add("perfil '$name': modelo '$($e['Name'])' sem Provider (ex.: openai, anthropic, ollama_chat)")
                }
            }
...
function Get-ClaudexSlots {
    # PURA. Os 3 atalhos do picker -> nome de modelo do catálogo. Aceita `Slots` (forma
    # completa) ou `Models.Opus/Sonnet/Haiku` (forma simples). Slot vazio é LEGÍTIMO agora:
    # significa "não remapeia este slot" — o Claude Code segue mandando o modelo Anthropic
    # original, que o catálogo pode inclusive atender se tiver uma entrada `anthropic`.
    param([Parameter(Mandatory)][hashtable]$ProfileObj)
    $slots = [ordered]@{}
    $src = $null
...
function New-ClaudexLiteLlmConfig {
    ...
    foreach ($e in $catalog) {
        ...
        foreach ($k in $e['Tuning'].Keys) { $tuning[[string]$k] = $e['Tuning'][$k] }
...
function New-ClaudexCliProxyConfig {
    ...
    #
    # AINDA NÃO VALIDADO AO VIVO: nada disto foi rodado contra o binário real — o `-Check`
    # segue avisando. Schema conferido contra o exemplo canônico é mais forte que leitura de
    # prosa, mas continua sendo doc, não execução. Só um lançamento real fecha isso.
    #
...
function New-ClaudexEngineConfig {
    ...
    switch ($engine) {
        'litellm'  { return New-ClaudexLiteLlmConfig -ProfileObj $ProfileObj -LocalKey $LocalKey }
...
function Start-ClaudexProxy {
    ...
    if (-not (Get-Command $cmd -ErrorAction SilentlyContinue)) {
        throw "motor '$($EngineSpec['Engine'])' não encontrado no PATH ('$cmd'). Instale com: $($EngineSpec['InstallHint'])"
    }
...
function Get-ClaudexEnvForProfile {
    ...
    $map = [ordered]@{ Opus = 'ANTHROPIC_DEFAULT_OPUS_MODEL'; Sonnet = 'ANTHROPIC_DEFAULT_SONNET_MODEL'; Haiku = 'ANTHROPIC_DEFAULT_HAIKU_MODEL' }
...
function Format-ClaudexList {
    ...
    foreach ($p in @($Profiles)) {
        ...
        elseif ($backend -eq 'proxy') {
            ...
            $prov = if ($p.ContainsKey('Provider')) { [string]$p['Provider'] } else { '?' }
            $port = try { Get-ClaudexEnginePort -ProfileObj $p } catch { '?' }
            $extra = " · $eng -> $prov · :$port"
            # Os modelos são a informação útil aqui: é o que dá p/ escolher no /model.
            $mdl = @(Get-ClaudexProfileModelList -ProfileObj $p)
            if ($mdl.Count -gt 0) { $extra += " · $($mdl -join ', ')" }
        }
        $lines.Add("  - $name [$backend]$extra")
    }
...
function Format-ClaudexDoctorLine {
    ...
    if ($backend -eq 'proxy') {
        ...
        if ($comChave.Count -gt 0) {
            ...
            $warn = if ($eng -eq 'cliproxy') { ' · schema de config PENDENTE de validação ao vivo' } else { '' }
            return "  [ok] $name [proxy/$eng] — motor presente · $($comChave.Count) segredo(s) por modelo resolvem$warn"
        }
        ...
        if ([string]::IsNullOrWhiteSpace($ref)) {
            ...
            return "  [ok] $name [proxy/$eng] — motor presente · sem SecretRef ($porque)"
        }
