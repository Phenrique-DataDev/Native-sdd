...
.PARAMETER Profile
  Perfil do scaffold: full (default) | mvp.

  full  Tudo — o scaffold completo (~35k tokens de rules always-on + hooks de processo). Certo para
        projeto que vai viver sob SDD por muito tempo.
  mvp   Enxuto, para protótipo/MVP: entrega só as rules do NÚCLEO (workflow-sdd, project-context,
        cli-first, tooling) e NÃO instala os 3 hooks de processo do projeto (curation-nudge,
        peer-heartbeat, complementary-repo-guard). Economia medida: ~45k chars
        (~20k tokens) de contexto always-on.
...
if ($Update) {
    ...
    if ($prev -and -not $PSBoundParameters.ContainsKey('ScaffoldProfile')) {
...
# --- Perfil mvp: tira a config de hooks do settings.json ------------------
# Os arquivos de hook não foram entregues (filtro de perfil), então a config que aponta para eles
# ficaria ÓRFÃ — o Claude Code tentaria rodar script inexistente a cada sessão. Config apontando para
# o vazio é pior que config ausente: falha barulhenta e recorrente por algo que o usuário pediu para
# não ter. Roda ANTES do manifest, de propósito: o hash precisa refletir o arquivo COMO FICOU.
    ...
    if (Test-Path -LiteralPath $settingsFile -PathType Leaf) {
        try {
            ...
            if ($sObj.PSObject.Properties['hooks']) {
                $sObj = Remove-JsonProperty -Object $sObj -Name 'hooks'
                $json = ($sObj | ConvertTo-Json -Depth 20)
                [System.IO.File]::WriteAllText($settingsFile, $json, [System.Text.UTF8Encoding]::new($false))
                Write-Step OK 'perfil mvp: settings.json sem hooks (nenhum hook de processo instalado)'
            }
        }
        catch {
