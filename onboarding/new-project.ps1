...
.PARAMETER Force
  Só com -Update: sobrescreve TAMBÉM os arquivos que você customizou ('conflito'/'customizado'/
  'desconhecido') e recria os que você dispensou ('dispensado'), sempre com backup .bak-*. Sem esta
  flag, arquivos customizados são PRESERVADOS e apenas listados, e os dispensados seguem fora.
...
.PARAMETER Profile
  Perfil do scaffold: mvp (default, desde 2026-09-19) | full.

  mvp   O DEFAULT. Entrega só as rules do NÚCLEO (workflow-sdd, project-context, cli-first, tooling),
        dos hooks de processo instala apenas o curation-nudge, e não entrega /orchestrate, /max nem
        as posturas que eles carregam. Economia medida com tools/rules-budget.ps1: ~33k chars
        (~14k tokens) de contexto always-on — 35.130 tok no full contra 20.843 no mvp.
  full  Tudo — o scaffold completo (~35k tokens de rules always-on + os 3 hooks de processo).
        Opt-in, para projeto que já sabe que vai viver sob SDD por muito tempo.

  POR QUE O DEFAULT TROCOU (auditoria externa da v0.10.61, 2026-09-19). Três medições independentes
  convergiram, e juntas dissolveram o trade-off que sustentava o full: em 9 tarefas pareadas o full
  custou +32,9% com obediência IDÊNTICA às três regras do núcleo (e com MENOS turnos, 26 contra 32);
  em fan-out, +40% por subagente, porque 73% do delta always-on se propaga a cada um; numa tarefa
  pequena, 3,6x o custo para entregável equivalente. Não é "economiza contexto mas perde disciplina":
  a disciplina medida é a mesma. Quem quiser o full tem uma flag; quem não sabe o que quer paga menos.
  O número por tamanho de sessão está no README — em contexto grande o overhead cai a ~3,5%, e aí
  este eixo deixa de decidir.

  O que o mvp NÃO corta (de propósito): o pre-commit anti-segredo (.githooks/) continua — é a única
  peça de segurança do scaffold, e MVP é justo onde se commita segredo com pressa. Os guards
  secret-guard/destructive-guard vivem em ~/.claude (nível de usuário) e são herdados de qualquer jeito.

  O perfil é GRAVADO em .claude/.scaffold-version e o -Update o respeita — sem isso o primeiro
  upgrade traria de volta tudo que o mvp cortou. Para migrar depois: nsp <path> -Update -Profile full.
...
param(
    [string]$Path = '.',
    # '$ScaffoldProfile' com Alias 'Profile': o usuário segue digitando `-Profile mvp`, mas a variável
    # NÃO se chama $Profile — isso colidiria com a automática $PROFILE (PSAvoidAssignmentToAutomaticVariable).
    # Default 'mvp' desde 2026-09-19 — ver o .PARAMETER Profile para as três medições que o trocaram.
    # Projeto JÁ CRIADO não muda de vida: o -Update herda o perfil do .scaffold-version (linha ~297),
    # e só um -Profile explícito o altera.
    [ValidateSet('full', 'mvp')][Alias('Profile')][string]$ScaffoldProfile = 'mvp',
    [switch]$Update,
    [switch]$Force,
    [switch]$AllowNested,
    [switch]$Git,
...
# --- Guarda de runtime: Windows PowerShell 5.1 -> relança em pwsh --------
# O 5.1 é uma PORTA DE ENTRADA legítima (é o `powershell.exe` que ainda abre por padrão em muita
# máquina), mas NÃO é um runtime suportado aqui: verificado em 2026-07-16, o script quebra nele
# (`[string[]]@($null)` -> $null só no 5.1, e o manifest não gravava — projeto sem baseline). Em vez
# de prometer compatibilidade que nenhum CI cobre, detectamos e reexecutamos tudo em pwsh.
# Este bloco só é alcançável porque o arquivo tem BOM UTF-8: sem ele o 5.1 lê o script como ANSI e o
# PARSER morre antes de qualquer linha rodar — inclusive esta. O BOM não é estilo, é o que faz o
# guard existir. Ver Get-RelaunchArgList em windows/lib.ps1.
if ($PSVersionTable.PSVersion.Major -lt 6) {
    $pwshCmd = Get-Command pwsh -ErrorAction SilentlyContinue
    if (-not $pwshCmd) {
        Write-Step FAIL "Windows PowerShell $($PSVersionTable.PSVersion) não é suportado, e o pwsh (PowerShell 7) não está instalado."
        Write-Step INFO 'Instale com: winget install Microsoft.PowerShell — ou rode o onboarding (onboarding\install.ps1), que já o instala.'
        exit 2
    }
    # Sem $pwshCmd.Version na mensagem: para um .exe o Get-Command devolve 0.0.0.0, e imprimir
    # "relançando em pwsh 0.0.0.0" é dado inventado. O caminho é verdade verificável; a versão não.
    Write-Step INFO "Windows PowerShell $($PSVersionTable.PSVersion) não é suportado — relançando em $($pwshCmd.Source)..."
    $fwd = Get-RelaunchArgList -Bound $PSBoundParameters -ScriptPath $PSCommandPath
    # O processo filho herda o diretório atual, então -Path relativo continua resolvendo igual.
    & $pwshCmd.Source @fwd
    exit $LASTEXITCODE
}
...
# Terminal de verdade = pode perguntar. SDD_ASSUME_INTERACTIVE=1 é o mesmo seam de teste já usado
# pela guarda de aninhamento abaixo (força este ramo com stdin vindo de pipe).
$script:IsInteractive = ($env:SDD_ASSUME_INTERACTIVE -eq '1') -or
                        ([Environment]::UserInteractive -and -not [Console]::IsInputRedirected)

# --- Nome do projeto: pergunta em vez de assumir --------------------------
# `nsp` sem -Path caía no default '.' e equipava a pasta atual EM SILÊNCIO — ótimo quando é o que
# você queria, confuso quando você só esqueceu o nome. Agora ele pergunta.
# Enter PRESERVA o comportamento antigo (equipar a pasta atual), então quem já usava `nsp` sozinho de
# dentro do projeto não perde nada — só ganha a chance de ver o que vai acontecer antes.
# Só na CRIAÇÃO interativa: -Update opera sobre projeto existente, e -Check/-DryRun não escrevem.
if (-not $Update -and -not $Check -and -not $DryRun -and
    -not $PSBoundParameters.ContainsKey('Path') -and $script:IsInteractive) {

    Write-Host ''
    Write-Host '  Nome do projeto ' -NoNewline
    Write-Host '(vira a pasta e o nome do projeto)' -ForegroundColor DarkGray
    Write-Host "  Enter = equipar a pasta atual ($((Get-Location).Path))" -ForegroundColor DarkGray
    $resposta = (Read-Host '  Nome').Trim()

    if ($resposta) {
        $nomeOk = Test-ProjectNameValid -Name $resposta
        if (-not $nomeOk.Valid) {
            Write-Step FAIL "nome inválido — $($nomeOk.Reason)"
            exit 2
        }
        $Path = $resposta
        Write-Step OK "projeto: $Path"
    }
    else {
        Write-Step OK "equipando a pasta atual: $((Get-Location).Path)"
    }
}

# --- Validação do nome (venha do prompt, de -Path, ou do default) ---------
# Vale também para quem passou -Path: um nome que o Windows recusa vira erro obscuro de filesystem
# lá na frente. Melhor falhar aqui, dizendo o porquê.
$nomeFinal = Split-Path -Leaf ($Path.TrimEnd('\', '/'))
$valid = Test-ProjectNameValid -Name $nomeFinal
if (-not $valid.Valid) {
    Write-Step FAIL "nome de projeto inválido ('$nomeFinal') — $($valid.Reason)"
    exit 2
}
...
if (-not $Update -and
    ...
    # Mesma verdade do prompt de nome acima — uma fonte só (duplicar o predicado é como o drift nasce).
...
if ($Update) {
    ...
    # O -Update HERDA o perfil gravado no projeto, salvo se o usuário pedir outro explicitamente.
    # Sem isto, um `nsp <path> -Update` num projeto mvp usaria o default 'full' e traria de volta
    # todas as rules cortadas — desfazendo a escolha do usuário em silêncio, que é exatamente o tipo
    # de "upgrade destrutivo" que a v0.8.18 existe para impedir.
    # PSBoundParameters usa o nome CANÔNICO do parâmetro ('ScaffoldProfile'), não o alias '-Profile':
    # mesmo quando o usuário digita `-Profile mvp`, a chave é 'ScaffoldProfile'.
    if (-not $PSBoundParameters.ContainsKey('ScaffoldProfile')) {
        if ($prev) {
            $ScaffoldProfile = $prev.Profile
            Write-Step INFO "perfil do projeto: $ScaffoldProfile (do .scaffold-version; use -Profile para mudar)"
        }
        else {
            # SEM marcador (projeto anterior à v0.8.29) o default NOVO não vale: 'mvp' aqui recortaria
            # o plano e faria os contratos anunciarem como ausente uma rule que está no disco desde
            # sempre. A ausência de marcador já é tratada como 'full' pelo Read-ScaffoldVersion; este
            # ramo é o mesmo fail-safe para quando não há marcador NENHUM.
            $ScaffoldProfile = 'full'
            Write-Step INFO 'sem perfil gravado (projeto anterior à v0.8.29): tratando como full — use -Profile mvp para enxugar'
        }
    }
    if (-not $prev) {
    ...
    # Filtro de PERFIL no PLANO, e não só na aplicação. Sem esta linha o plano era computado sobre o
    # template INTEIRO enquanto a escrita usava o $map filtrado (mais abaixo): num projeto 'mvp' o
    # -Update -DryRun anunciava 18 ações e executava 5, e as 14 fantasmas eram exatamente o que o
    # perfil corta. A leitura correta daquele plano era "o upgrade vai desfazer minha escolha de
    # perfil" — e a reação correta, abortar. O comportamento sempre esteve certo; a comunicação não.
    # Mesma função e mesmo $ScaffoldProfile já resolvido acima (herança do .scaffold-version), então
    # `-Update -Profile full` a partir de um mvp continua enxergando tudo: lá o predicado é constante.
    #
    # E o filtro vale SÓ para o que vem do template. Filtrar o plano inteiro desligava a RETIRADA:
    # um arquivo que nós entregamos no passado e que hoje está fora do perfil sumia do plano e ficava
    # no disco para sempre — o corte de always-on nunca alcançaria quem migra de full para mvp, e a
    # classe 'removido' viraria meia-feature. Quem pegou isso foi o E2E de retirada que já existia
    # (scaffold-upgrade.Tests.ps1), não a leitura do código.
    ...
    # 'customizado' e 'dispensado' entram porque o template TEM o arquivo: sem elas aqui, o -Force
    # deixaria de alcançá-las — e o -Force é justamente a porta de volta que a mensagem das duas
    # oferece ao usuário.
    $comFonte = @('novo', 'intacto', 'em-dia', 'conflito', 'desconhecido', 'customizado', 'dispensado')

    $upgrade = @($upgrade | Where-Object {
            $_.Class -notin $comFonte -or (Test-ScaffoldRelInProfile -Rel $_.Rel -Profile $ScaffoldProfile)
        })
    ...
    Write-Step INFO "Plano: $(@($write).Count) a atualizar, $(@($del).Count) a remover, $(@($hold).Count) preservado(s), $(@($upgrade | Where-Object Class -eq 'dispensado').Count) dispensado(s), $(@($upgrade | Where-Object Class -eq 'em-dia').Count) em dia, $(@($ger).Count) gerado(s)"
    ...
    # As três razões para NÃO tocar são diferentes, e dizê-las com a mesma frase custava trabalho
    # inventado: 'customizado' mandava comparar com um upstream idêntico ao baseline (nada a
    # reaplicar), e 'orfao' oferecia -Force para uma classe em que o -Force não faz nada.
    $holdConflito    = @($hold | Where-Object Class -in @('conflito', 'desconhecido'))
    $holdCustomizado = @($hold | Where-Object Class -eq 'customizado')
    $holdOrfao       = @($hold | Where-Object Class -eq 'orfao')
    $dispensados     = @($upgrade | Where-Object Class -eq 'dispensado')

    if ($holdConflito.Count -gt 0) {
        Write-Host ''
        Write-Step WARN 'NÃO serão tocados — você editou estes arquivos E o template também mudou:'
        foreach ($p in $holdConflito) { Write-Step WARN "  [$($p.Class)] $($p.Rel)" }
        if ($Force) { Write-Step WARN '-Force: serão SOBRESCRITOS mesmo assim (com backup .bak-*).' }
        else { Write-Step INFO 'Compare à mão e reaplique o que quiser; ou use -Force para sobrescrever (com backup).' }
    }
    if ($holdCustomizado.Count -gt 0) {
        Write-Host ''
        Write-Step OK 'PRESERVADOS — customização sua; o template não mudou, nada a reaplicar:'
        foreach ($p in $holdCustomizado) { Write-Step OK "  [customizado] $($p.Rel)" }
        if ($Force) { Write-Step WARN '-Force: serão SOBRESCRITOS mesmo assim (com backup .bak-*) — sua edição volta ao template.' }
    }
    if ($holdOrfao.Count -gt 0) {
        Write-Host ''
        Write-Step WARN 'PRESERVADOS — o template REMOVEU estes arquivos, mas você os editou (decisão sua):'
        foreach ($p in $holdOrfao) { Write-Step WARN "  [orfao] $($p.Rel)" }
        Write-Step INFO 'Apague-os à mão se não quiser mais; o -Force não os toca (não há fonte para repor).'
    }
    if ($dispensados.Count -gt 0) {
        Write-Host ''
        Write-Step SKIP 'DISPENSADOS — nós entregamos, você apagou; não vamos recriar:'
        foreach ($p in $dispensados) { Write-Step SKIP "  [dispensado] $($p.Rel)" }
        Write-Step INFO 'Use -Force para trazê-los de volta.'
    }
}
Write-Host ''
...
# Filtro de PERFIL: no 'mvp' o template entrega menos. Aplicado ao $map, então vale para tudo que vem
# depois — inclusive o MANIFEST, que passa a registrar só o que realmente entregamos (é o contrato:
# "o manifest é o que NÓS escrevemos"). No 'full' o predicado é sempre verdadeiro: nada muda.
$RemovidosDoPerfil = @()
if ($ScaffoldProfile -eq 'mvp') {
    $antes = @($map).Count
    # A lista do que SAI é guardada, não só contada: quem cita esses arquivos (AGENTS.md, CLAUDE.md,
    # commands) é reescrito mais abaixo. Sem isso o projeto nasce com o contrato apontando para o vazio.
    $RemovidosDoPerfil = @($map | Where-Object { -not (Test-ScaffoldRelInProfile -Rel $_.Rel -Profile $ScaffoldProfile) } | ForEach-Object { $_.Rel })
    $map = @($map | Where-Object { Test-ScaffoldRelInProfile -Rel $_.Rel -Profile $ScaffoldProfile })
    $cortados = $antes - @($map).Count
    Write-Step INFO "perfil mvp: $cortados arquivo(s) fora (rules opcionais + hooks de processo); o pre-commit anti-segredo continua"
}
...
# --- Perfil mvp: o settings.json só aponta para hook que foi entregue -----
# Config apontando para o vazio é pior que config ausente: o Claude Code tentaria rodar script
# inexistente a cada sessão. Até 2026-09-19 a chave `hooks` inteira era apagada — correto enquanto
# NENHUM hook vinha. Com o curation-nudge entrando no perfil, apagar tudo desligaria justamente o
# mecanismo que ele passou a precisar; a poda virou seletiva, pelo que de fato ficou fora.
# Roda ANTES do manifest, de propósito: o hash precisa refletir o arquivo COMO FICOU.
if ($ScaffoldProfile -eq 'mvp' -and -not $Check -and -not $DryRun) {
    $hooksFora = @($RemovidosDoPerfil | Where-Object { $_ -like '*.claude\hooks\*' -or $_ -like '*.claude/hooks/*' } |
            ForEach-Object { Split-Path $_ -Leaf })
    $settingsFile = Join-Path $DestRoot '.claude\settings.json'
    if ($hooksFora.Count -gt 0 -and (Test-Path -LiteralPath $settingsFile -PathType Leaf)) {
        try {
            $sObj = (Get-Content -LiteralPath $settingsFile -Raw) | ConvertFrom-Json
            if ($sObj.PSObject.Properties['hooks']) {
                $sObj = Remove-JsonHookEntry -Object $sObj -MissingFileName $hooksFora
                $json = ($sObj | ConvertTo-Json -Depth 20)
                [System.IO.File]::WriteAllText($settingsFile, $json, [System.Text.UTF8Encoding]::new($false))
                Write-Step OK "perfil mvp: settings.json sem as entradas de $($hooksFora.Count) hook(s) não entregue(s)"
            }
        }
        catch {
            Write-Step WARN "perfil mvp: não consegui limpar os hooks do settings.json — $($_.Exception.Message)"
            $summary.Warn++
        }
    }
}

# --- Perfil mvp: os CONTRATOS param de citar o que o perfil não entregou --
# Sem isto, um projeto 'mvp' nascia com 51 links quebrados e um AGENTS.md mandando seguir 8 rules
# ausentes: o primeiro /check do usuário saía vermelho sem culpa dele, e o contrato canônico descrevia
# outro projeto. Roda ANTES do manifest pelo mesmo motivo do bloco acima: o hash gravado tem de ser o
# do arquivo COMO FICOU, senão o próximo -Update classifica a nossa própria reescrita como 'conflito'.
# Varre TODO .md entregue, e não uma lista de arquivos "importantes": a primeira versão deste bloco
# cobria AGENTS.md, CLAUDE.md e iterate.md, e a medição achou os outros 37 links quebrados espalhados
# por commands, posturas, skills e `kb/_ABOUT.md`. Quem cita uma rule não é um clube fechado.
if ($ScaffoldProfile -eq 'mvp' -and -not $Check -and -not $DryRun -and @($RemovidosDoPerfil).Count -gt 0) {
    $reescritos = 0
    foreach ($alvo in @($map | Where-Object { $_.Rel -like '*.md' } | ForEach-Object { $_.Dst })) {
        if (-not (Test-Path -LiteralPath $alvo -PathType Leaf)) { continue }
        $rel = $alvo.Substring($DestRoot.Length).TrimStart('\', '/')
        try {
            $antesTxt = [System.IO.File]::ReadAllText($alvo)
            $depoisTxt = Get-ScaffoldProfileRewrite -Text $antesTxt -RemovedRel $RemovidosDoPerfil -ProfileName $ScaffoldProfile
            if ($depoisTxt -ne $antesTxt) {
                [System.IO.File]::WriteAllText($alvo, $depoisTxt, [System.Text.UTF8Encoding]::new($false))
                $reescritos++
            }
        }
        catch {
            Write-Step WARN "perfil mvp: não consegui reescrever ${rel} — $($_.Exception.Message)"
            $summary.Warn++
        }
    }
    if ($reescritos -gt 0) {
        Write-Step OK "perfil mvp: $reescritos contrato(s) reescrito(s) — nenhum aponta para arquivo que este perfil não entrega"
    }
}
...
elseif ($DryRun) { Write-Step DRY  "escreve: .claude\.scaffold-version (template_version: $FrameworkVersion)" }
    ...
    try {
        $content = Get-ScaffoldVersionContent -RepoRoot $RepoRoot -Commit $commit -Version $FrameworkVersion -Profile $ScaffoldProfile
...
# --- pre-commit EXECUTÁVEL (Linux/macOS) ----------------------------------
# O git IGNORA hook sem bit de execução — só um "hint" no stderr — e o pre-commit anti-segredo ficava
# inativo em SILÊNCIO no Unix: o commit com token passava (runner macos-15 e as 3 distros do e2e
# Linux, 2026-09-10). A cópia não garante o bit (o template era 100644 no git; no Windows o bit nem
# existe). Roda também no -Update: conserta o disco de projeto criado antes disto.
$preCommit = Join-Path (Join-Path $DestRoot '.githooks') 'pre-commit'
if ((Get-OnboardingOS) -ne 'Windows' -and -not $Check -and -not $DryRun -and
    (Test-Path -LiteralPath $preCommit -PathType Leaf)) {
    & chmod +x $preCommit 2>$null
    if ($LASTEXITCODE -ne 0) {
        Write-Step WARN 'nao consegui marcar .githooks/pre-commit como executavel — rode: chmod +x .githooks/pre-commit'
        $summary.Warn++
    }
}
...
if ($Git) {
    ...
    elseif ($DryRun) { Write-Step DRY  'git init + commit inicial' }
        ...
        # --initial-branch=main (2026-09-19): sem isto a branch ficava a cargo do init.defaultBranch
        # do usuário — ausente numa máquina nova —, e o projeto nascia em 'master' nos dois SOs. O
        # AGENTS.md que este mesmo comando acabou de entregar diz "main protegida: trabalho em branch
        # de feature", e o agente git-workflow repete três vezes que não se toca na main: a guarda
        # mais importante do scaffold apontava para uma branch que não existia no projeto. Regra sem
        # alvo é regra que não roda. Suportado desde git 2.28; o produto já exige gh e winget.
        git -C $DestRoot init --quiet --initial-branch=main 2>&1 | Out-Null
        if ($LASTEXITCODE -ne 0) {
            ...
            # O hook entra no commit inicial como 100755 em QUALQUER SO: no Windows o git grava arquivo
            # novo como 100644, e o clone num Mac/Linux teria o pre-commit inativo em silêncio.
            if (Test-Path -LiteralPath $preCommit -PathType Leaf) {
                git -C $DestRoot update-index --chmod=+x -- .githooks/pre-commit 2>&1 | Out-Null
            }
