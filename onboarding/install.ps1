...
.PARAMETER WithClaudex
  Opt-in: provisiona o `claudex` (troca fina de modelo/provider Anthropic-compatível) user-scoped —
  semeia ~/.claude/claudex/profiles.psd1, cria secrets/ com ACL restrita ao usuário e registra a
  função `claudex` no $PROFILE (mesmo molde de nsp). Camada FINA: PowerShell puro, sem proxy nem
  OAuth de terceiro (fase futura gated), zero dependência externa. Idempotente (não sobrescreve um
  profiles.psd1 já customizado). Não bloqueante (WARN). Reabra o terminal p/ carregar `claudex`.

.PARAMETER WithCliProxy
  Opt-in: provisiona o binário `cli-proxy-api` (CLIProxyAPI, de terceiro — repo público
  router-for-me/CLIProxyAPI), o motor que o claudex usa p/ usar modelo por LOGIN DE ASSINATURA
  (OAuth) em vez de chave de API. Baixa o asset PINADO (onboarding/cliproxy/cliproxy.psd1),
  VERIFICA o SHA-256 antes de instalar, extrai e instala em ~/.claude/tools/cliproxy/<versão>.
  NÃO faz login (ato do usuário: `claudex -Login claude|codex|gemini`). A config que o claudex
  gera desliga o cloaking do motor por construção — ver a nota em New-ClaudexCliProxyConfig.
  Idempotente, marcador ~/.claude/.native-sdd-cliproxy-version. Não bloqueante (WARN).

.PARAMETER WithHerdr
  Opt-in: provisiona o binário `herdr` (multiplexador de agentes terminal-native, de terceiro —
  repo público ogulcancelik/herdr) user-scoped. Baixa o asset da versão PINADA (onboarding/herdr/
  ...
  Não bloqueante (WARN). Windows x64 é o alvo primário. Docs: https://herdr.dev/docs/
...
.EXAMPLE
  ...
  .\install.ps1 -WithClaudex                   # + claudex (troca fina de modelo/provider)
  .\install.ps1 -Check -WithClaudex            # simula o provisionamento do claudex
  .\install.ps1 -WithHerdr                     # + binário herdr (multiplexador de agentes, verifica SHA-256)
  .\install.ps1 -WithCliProxy                  # + motor de login por assinatura (OAuth) p/ o claudex
...
if ($onWindows) {
    & (Join-Path $PSScriptRoot 'windows\apply.ps1') -SkipClis:$SkipClis -Check:$Check -DryRun:$DryRun `
        ...
        -WithClaudex:$WithClaudex -WithHerdr:$WithHerdr -WithCliProxy:$WithCliProxy `
        -SetDefaultShell:$SetDefaultShell -KeepRetired:$KeepRetired -Force:$Force
    exit $LASTEXITCODE
}
elseif ($IsMacOS) {
