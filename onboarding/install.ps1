...
.PARAMETER ExtraPlugins
  Opt-in: instala plugins extra user-scoped ("global dormente"): ui-ux-pro-max (design) e
  visual-explainer (reporting). Disponíveis em todo projeto, auto-ativam só no trabalho da
  categoria. Off por padrão p/ manter o scaffold context-free. Não bloqueante (falha = WARN).
...
.EXAMPLE
  ...
  .\install.ps1 -ExtraPlugins        # + plugins user-scoped (ui-ux-pro-max, visual-explainer)
...
if ($onWindows) {
    & (Join-Path $PSScriptRoot 'windows\apply.ps1') -SkipClis:$SkipClis -Check:$Check -DryRun:$DryRun `
        -NonInteractive:$NonInteractive -ManagedPolicy $ManagedPolicy -ExtraPlugins:$ExtraPlugins
    exit $LASTEXITCODE
}
elseif ($IsMacOS) {
