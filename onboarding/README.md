...
macOS: Homebrew + tarball oficial do `pwsh`, validado no CI no runner arm64 `macos-26` a cada mudança
e no `macos-15` sob demanda).
...
> o framework via `$env:SDD_WORKFLOW_HOME` — reinstale se mover o clone. A mesma var vai também
> para o `env` do `~/.claude/settings.json`, que a entrega às sessões do Claude Code abertas fora
> de um terminal (app desktop, IDE), onde o `$PROFILE` não é lido.
>
> **`nsp` não reconhecido?** Você está num **Windows PowerShell 5.1**, onde o atalho não existe. Isso
> acontece também **dentro do herdr**, mesmo chamado a partir do pwsh 7: os panes dele não herdam o shell
> de quem os abriu e, no Windows, caem no 5.1. `install.ps1 -SetDefaultShell` (ou `-WithHerdr`) grava
> `default_shell = "pwsh.exe"` no config do herdr. Panes já abertos continuam no shell antigo, então abra
> um novo. No meio-tempo, digite `pwsh` no pane.
...
├── windows/
│   ├── apply.ps1               (orquestra A1 + A2 + A2c + A2e + A2d; SO-agnóstico no miolo)
